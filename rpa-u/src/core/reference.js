...
export function createReferenceResolver({ identity, registry, linker, sources = null, emit = null, clock = () => Date.now() } = {}) {
  ...
  async function fetch(typeId, entityId, fieldKey) {
    ...
    if (emit && reference.owner) {
      await emit("sync.requested", { connector: reference.owner, entityType: typeId, entityId, fields: [fieldKey] }, { source: "ru", subject: { entityType: typeId, entityId } });
    }
    ...
    if (emit && reference.owner) {
      await emit("sync.completed", { jobId, connector: reference.owner, entityType: typeId, entityId, changes: differs ? 1 : 0 }, { source: "ru", subject: { entityType: typeId, entityId } });
    }
  ...
  async function refresh(typeId, entityId, fieldKey, { adopt = true } = {}) {
    ...
    const sourceConnector = result.fetched.source === "owner" ? result.owner : "ru";
