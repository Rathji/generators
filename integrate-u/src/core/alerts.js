...
export function createAlertManager({ db = null, collection = "alerts", emit = null, clock = () => Date.now(), limit = 200 } = {}) {
  ...
  async function raise({
    ...
    if (emit) {
      await emit(
        ...
        { source: "iu", subject: alert.entity ? { entityType: alert.entity.typeId, entityId: alert.entity.id } : null }
      );
    }
  ...
  async function acknowledge(id, { actor = "operator" } = {}) {
    ...
    await persist(alert);
    if (emit) await emit("alert.acknowledged", { alertId: alert.key, actor }, { source: "iu", subject: alert.entity ? { entityType: alert.entity.typeId, entityId: alert.entity.id } : null });
  ...
  async function clear(id, { reason = "" } = {}) {
    ...
    await persist(alert);
    if (emit) await emit("alert.cleared", { alertId: alert.key, reason }, { source: "iu", subject: alert.entity ? { entityType: alert.entity.typeId, entityId: alert.entity.id } : null });
    return { ok: true, alert, reused: false };
  }
