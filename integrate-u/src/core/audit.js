...
export function createAuditMonitor({
  ...
  function describe(event) {
    ...
    let movement = null;
    if (direction === "in") movement = { from: event.source, to: "iu", kind: "inbound" };
    else if (direction === "out") movement = { from: "iu", to: payload.target || payload.connector || "downstream", kind: "outbound" };
    const field = payload.field || (Array.isArray(payload.fields) && payload.fields.length === 1 ? payload.fields[0] : null);
    const actor = payload.actor || (event.source === "iu" ? "hub" : event.source);
    const entry = {
      id: event.id,
      seq: event.seq,
      at: event.time,
  ...
  function lifecycle(typeId, entityId) {
    ...
    const connectors = Array.from(new Set(list.flatMap((entry) => [entry.source, entry.movement?.from, entry.movement?.to]).filter((value) => value && value !== "iu")));
  ...
  function facets() {
    ...
    return {
      ...
      connectors: tally(entries.flatMap((entry) => [entry.source, entry.movement?.from, entry.movement?.to]).filter((value) => value && value !== "iu")),
  ...
  async function note({ message, actor = "operator" } = {}) {
    ...
    const result = await emit("audit.note", { message: text, actor }, { source: "iu" });
