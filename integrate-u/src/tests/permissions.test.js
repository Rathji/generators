...
suite("Permission mapping", () => {
  ...
  test("the owner role is unrestricted", () => {
    assert(permissions.hasCapability({ connector: "iu", toolRole: "Hub Owner" }, "registry.write"));
    assert(permissions.hasCapability({ connector: "iu", toolRole: "Hub Owner" }, "capability.that.does.not.exist"));
  });
