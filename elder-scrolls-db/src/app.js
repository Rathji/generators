...
const ROADMAP = [
  {
    ...
    steps: [
      { kind: 'auto', label: 'Register the game in games.json (id, title, subtitle, year, era, province, engine)', done: (g) => !!(g.id && g.title && g.year) },
  ...
  },
  {
    ...
    steps: [
      { kind: 'auto', label: 'Extract & normalize records for each category (see grid below)', done: (g, c) => c.categoriesIngested > 0 },
      { kind: 'auto', label: 'Validate every record against the canonical schema (required fields present)', done: (g, c) => c.requiredMissing === 0 },
      { kind: 'auto', label: 'Commit each category to src/data/\u003cgame\u003e/\u003ccategory\u003e.json', done: (g, c) => c.categoriesIngested > 0 },
      { kind: 'auto', label: 'Verify records render and are searchable in-app', done: (g, c) => c.categoriesIngested > 0 }
    ]
  },
  {
    id: 'P4', name: 'Cross-linking', desc: 'Bind the categories together so the archive behaves as one living graph.',
    steps: [
      { kind: 'auto', label: 'Link records across entity types by id (NPC \u2194 faction \u2194 location \u2194 quest)', done: (g, c) => c.linksTotal > 0 },
      { kind: 'auto', label: 'Resolve every reference \u2014 no orphan ids left dangling', done: (g, c) => c.linksTotal > 0 && c.linksResolved === c.linksTotal },
      { kind: 'auto', label: 'Confirm search covers the game\u2019s whole dataset', done: (g, c) => c.total > 0 }
    ]
  },
  {
    id: 'P5', name: 'Quality & Lore', desc: 'Make every record worth reading and trustworthy.',
    steps: [
      { kind: 'auto', label: 'Write lore/description text for the majority of records', done: (g, c) => c.total > 0 && c.withLore / c.total >= 0.6 },
      { kind: 'manual', label: 'Fact-check records against the recorded sources', key: 'p5-factcheck' },
      { kind: 'auto', label: 'Mark every record verified in its status field', done: (g, c) => c.total > 0 && c.verifiedRatio === 1 },
  ...
  },
  {
    ...
    steps: [
      { kind: 'auto', label: 'Set game status to \u2018complete\u2019 in the registry', done: (g) => g.status === 'complete' },
      { kind: 'auto', label: 'Bump the dataset version', done: (g) => !!g.version },
      { kind: 'auto', label: 'Snapshot & ship (the data is part of the generator)', done: (g) => g.status === 'complete' }
    ]
  }
];
...
function computeContext(game) {
  const c = {
    ...
    categoriesIngested: 0, requiredMissing: 0,
...
function resolveRef(gameId, type, id) {
  const rec = (state.data[gameId]?.[type] || []).find((r) => r.id === id);
  if (rec) return { gameId, rec };
  for (const g of state.games) {
    const r2 = (state.data[g.id]?.[type] || []).find((r) => r.id === id);
  ...
  <header class="archive-header">
    ...
    <div class="controls">
      <div class="search-wrap">
        ...
        <input id="searchInput" type="search" placeholder="Search all games\u2026" autocomplete="off">
      </div>
      <div class="tabs">
    ...
    <div class="main-grid">
      ...
      <div class="content">
        <div class="content-top">
          ...
          <span class="sub" id="contentSub"></span>
        </div>
...
function renderContent() {
  ...
  $('contentSub').textContent = state.query
    ? `matching \u201c${state.query}\u201d across every game`
    : `${game.subtitle} \u00b7 ${game.era} \u00b7 ${recs.length} record${recs.length === 1 ? '' : 's'} \u00b7 ${type.description}`;
    ...
    for (const g of state.games) {
      for (const t of state.schema.entityTypes) {
        for (const r of state.data[g.id]?.[t.key] || []) {
          ...
          if (hay.includes(state.query)) results.push({ game: g, type: t, rec: r });
        }
      }
    }
  }
  ...
  if (state.query) {
    ...
    grid.innerHTML = results.map(({ game: g, type: t, rec }) => cardHTML(rec, t, g, true)).join('');
    grid.querySelectorAll('.entity-card').forEach((card) => card.addEventListener('click', () => {
      const { g, t, id } = card.dataset;
      openDetail(id, t, g);
    }));
    return;
  }
...
function cardHTML(rec, type, game, showGame) {
  ...
  const gameTag = showGame ? `<span class="ec-type">${escapeHtml(game.title)}</span>` : '';
  return `<div class="entity-card" data-g="${game.id}" data-t="${type.key}" data-id="${escapeHtml(rec.id)}">
    <div class="ec-top">
    ...
    ${gameTag}
    <p class="ec-desc">${escapeHtml(desc)}</p>
...
function openDetail(id, typeKey, gameId) {
  ...
  <div class="modal-backdrop" id="modalBackdrop">
    <div class="scroll">
      <button class="close-btn" id="modalClose" title="Close">\u00d7</button>
  ...
  $('modalRoot').querySelectorAll('.chip[data-link]').forEach((chip) => chip.addEventListener('click', () => {
...
function onEsc(e) { if (e.key === 'Escape') { closeDetail(); document.removeEventListener('keydown', onEsc); } }
function closeDetail() { $('modalRoot').innerHTML = ''; state.detail = null; }
...
function renderField(f, rec, gameId) {
  ...
  if (f.type === 'links') {
    ...
    const chips = ids.map((id) => {
      ...
      return `<span class="chip ${res ? '' : 'missing'}" ${res ? `data-link data-target="${f.target}" data-id="${escapeHtml(id)}"` : ''}>${escapeHtml(name)}</span>`;
    }).join('');
