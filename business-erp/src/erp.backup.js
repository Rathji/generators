...
(function () {
  ...
  const BACKUP_NAME = "erp-v1-backup";
  ...
  backup.backupBundle = async function () {
    ...
    return {
      schema: "erp-backup",
      version: 1,
      generator: window.generatorName || "business-erp",
  ...
  backup.publishedBackup = async function () {
    ...
    if (r.doc && r.doc.records && r.doc.records[0] && r.doc.records[0].schema === "erp-backup") return r.doc.records[0];
    return null;
  };

  /* Validate a bundle. Returns { valid, errors: [{doc, error}], docs }.
     A document is only included if it parses as a versioned erp-doc. */
  backup.validateBundle = function (obj) {
    const errors = [];
    if (!obj || obj.schema !== "erp-backup") {
      return { valid: false, errors: [{ doc: "(bundle)", error: "Not an ERP backup — expected schema \"erp-backup\"." }], docs: {} };
    }
    const docs = {};
    for (const name of Object.keys(obj.docs || {})) {
      const d = obj.docs[name];
      if (!d || d.schema !== "erp-doc") {
        errors.push({ doc: name, error: "Skipped: not a versioned erp-doc." });
        continue;
      }
  ...
  backup.restoreArchive = async function (archiveId) {
    ...
    const w = await store.forceSet(name, {
      schema: "erp-doc", schemaVersion: 1, doc: mod.doc.name, year: entry.originYear, rev: 0,
  ...
  backup.renderPanel = async function (el) {
    ...
    const render = async () => {
      ...
      const lastPub = published
        ...
        : '<p class="erp-alert">No published backup yet. Publish one so another device can restore from the ERP\'s own namespace.</p>';
      pane("backup").innerHTML =
        ui.card("Full backup", lastPub +
          "<p class=\"erp-alert\">Download saves every module document to a JSON file on your device. Publish stores the same bundle in the ERP's own storage namespace (re-publishing identical content is a safe no-op).</p>" +
      ...
      ui.bind(el, "click", "[data-act]", async (t, e, act) => {
        if (act === "download-backup") {
          ...
          try {
            ...
            ERP.toast(res.noop ? "Backup already published (identical content)." : "Backup published to the ERP namespace.", "success");
            render();
