...
(function () {
  ...
  function distroInfo() {
    try {
      ...
      return { name: "Webuntu Business", codename: "Iron Ledger", version: "12", basedOn: "Debian 12",
  ...
  function renderSection(name) {
    ...
    if (name === "appearance") {
      ...
      const openProjects = el("button", "set-btn", "Open Project Master");
      openProjects.type = "button";
      openProjects.addEventListener("click", () => { if (window.Apps) window.Apps.launch("project-master"); });
  ...
  async function devTasksFill(body) {
    try {
      ...
      if (!ids.length) { body.textContent = ""; body.appendChild(el("div", "set-note", "No project tasks yet — plan work in Project Master (Planning).")); return; }
      const rows = [];
      for (const id of ids) {
        const tasks = await window.Projects.loadTasks(id);
        if (!body.isConnected) return;
