(function () {
  ...
  function mockEnv(opts) {
    ...
    const ns = opts.ns || "bk" + BS.randHex(6);
  ...
  T.register("backup: build captures every module, its revision and this device's write keys", async () => {
    const env = mockEnv();
    const c1 = { records: [{ id: 1, name: "Acme" }], settings: { nextNumber: 2 } };
    await seedDoc(env.store, "companies", c1);
    await seedDoc(env.store, "contacts", { records: [] });
    ...
    if (snap.k !== "bcrmbak" || snap.kind !== "full" || snap.modules.length !== 6) return { pass: false, detail: "snapshot shape wrong" };
    const comp = snap.modules.find(m => m.module === "companies");
    if (!comp || !comp.present || comp.revision !== 1 || !eq(comp.content, c1)) return { pass: false, detail: "companies entry wrong" };
    const absent = snap.modules.filter(m => !m.present);
    if (absent.length !== 4 || absent.some(m => m.revision !== 0)) return { pass: false, detail: "absent entries wrong: " + absent.length };
    const headName = env.store.fileName("companies");
    ...
    return { pass: true, detail: "6 modules, 2 present (companies+contacts), key captured for " + headName };
  });

  T.register("backup: validate accepts the JSON file and rejects tampering", async () => {
    const env = mockEnv();
    await seedDoc(env.store, "companies", { records: [{ id: 1 }] });
    const built = await env.store.buildBackup();
    ...
    const comp = tampered.modules.find(m => m.module === "companies");
    comp.content.records.push({ id: 99, sneaky: true });
  ...
  T.register("backup: publish writes module files + index and is idempotent", async () => {
    const env = mockEnv();
    const c1 = { records: [{ id: 1, name: "Acme" }] };
    const d1 = { records: [{ id: 1, amount: 10 }], title: "pipe" };
    await seedDoc(env.store, "companies", c1);
    await seedDoc(env.store, "deals", d1);
    ...
    for (const m of ["companies", "deals"]) {
      if (!env.files.has(env.store.backupHeadName(m))) return { pass: false, detail: m + " backup file missing" };
    }
    if (env.files.has(env.store.backupHeadName("leads"))) return { pass: false, detail: "leads backup published though absent" };
    const rp = await env.store.readPublishedBackup();
    if (!rp.ok) return { pass: false, detail: "readPublished failed: " + JSON.stringify(rp) };
    const comp = rp.snapshot.modules.find(m => m.module === "companies");
    if (!comp || comp.revision !== 1 || !eq(comp.content, c1)) return { pass: false, detail: "published companies wrong" };
    ...
    const dealsBefore = env.files.get(env.store.backupHeadName("deals")).text;
    const c2 = { records: [{ id: 1, name: "Acme" }, { id: 2, name: "Beta" }] };
    const upd = await env.store.saveDoc("companies", c2);
    if (!upd.ok) return { pass: false, detail: "companies update failed" };
    ...
    const comp3 = JSON.parse(env.files.get(env.store.backupHeadName("companies")).text);
    if (comp3.revision !== 2 || !eq(comp3.content, c2)) return { pass: false, detail: "companies backup not updated to rev 2" };
    if (env.files.get(env.store.backupHeadName("deals")).text !== dealsBefore) return { pass: false, detail: "unchanged deals backup was rewritten" };
    const pub4 = await env.store.publishBackup();
    if (!pub4.ok || !pub4.noop) return { pass: false, detail: "second no-op expected" };
    return { pass: true, detail: "published 2 modules; unchanged re-publish is a no-op; one module update republishes just that module" };
  });
  ...
  T.register("backup: preview flags rollback, noop and pending blocks", async () => {
    ...
    const row2 = pv2.rows.find(r => r.module === "companies");
    if (!row2 || row2.action !== "noop") return { pass: false, detail: "current backup should preview as noop: " + JSON.stringify(row2) };
    await env.store.stageEdit("companies", { records: [{ id: 1 }, { id: 2 }, { id: 3 }] });
    const pv3 = await env.store.previewBackup(built.snapshot);
    if (!pv3.blocking || pv3.blocking.indexOf("companies") === -1) return { pass: false, detail: "pending edit not flagged: " + JSON.stringify(pv3.blocking) };
  ...
  T.register("backup: restore replaces live docs at a new revision and archives the old", async () => {
    ...
    const row = res.results["companies"];
    ...
    const hist = await env.store.listHistory("companies");
    ...
    const ld = await env.store.loadDoc("companies");
    ...
    if (!res2.ok || res2.results["companies"].action !== "noop") return { pass: false, detail: "second restore should be a no-op" };
    const h2 = headOf(env, env.store, "companies");
    const hist2 = await env.store.listHistory("companies");
  ...
  T.register("backup: restore is refused while a staged edit is pending", async () => {
    ...
    await seedDoc(env.store, "companies", c1);
    const built = await env.store.buildBackup();
    await env.store.stageEdit("companies", { records: [{ id: 1 }, { id: 2 }] });
    const res = await env.store.restoreBackup(built.snapshot);
    if (res.ok || res.code !== "pending_changes" || (res.modules || []).indexOf("companies") === -1) {
  ...
  T.register("backup: restore creates documents in a fresh namespace", async () => {
    ...
    await seedDoc(env.store, "companies", c1);
    const built = await env.store.buildBackup();
    const other = env.freshNs("bkz" + BS.randHex(6));
    ...
    const h = headOf(env, other.store, "companies");
  ...
  T.register("backup: a downloaded backup hands write ownership to a keyless device", async () => {
    ...
    if (!res.ok) return { pass: false, detail: JSON.stringify(res) };
    if (res.results["companies"].action !== "noop" || res.keysApplied < 1) {
      return { pass: false, detail: "expected noop with keys recovered: " + JSON.stringify(res).slice(0, 240) };
    }
    const upd = await dev.store.saveChecked("companies", { records: [{ id: 1, name: "Acme" }, { id: 2 }] }, { expectedBase: 1 });
  ...
  T.register("backup: a published backup carries no write keys", async () => {
    ...
    const upd = await dev.store.saveChecked("companies", { records: [{ id: 1 }, { id: 2 }] }, { expectedBase: 1 });
  ...
  T.register("backup: backup zone renders on the dashboard and rejects a bad file", async () => {
    if (!window.CRM_BACKUP || !window.CRM || !window.CRM.store) return { pass: true, skip: true, detail: "backup UI not loaded" };
    await window.CRM.go("dashboard");
    await window.CRM.ready();
    const zone = document.querySelector(".backup-zone");
    if (!zone) return { pass: true, skip: true, detail: "no store on this dashboard" };
    ...
    if (!chip || chip.textContent.indexOf(window.CRM.store.modules.length + " modules") === -1) return { pass: false, detail: "module chip wrong: " + (chip && chip.textContent) };
    ...
    return { pass: true, detail: "zone rendered with " + window.CRM.store.modules.length + " modules; bad file rejected inline" };
  });
  ...
  T.register("backup: real publish → newer edit → restore round-trip (needs saved generator)", async () => {
    ...
    const ns = "bkv" + Date.now().toString(36) + BS.randHex(4);
    let store;
    try {
      store = BS.create({
        ns,
    ...
    try {
      ...
      const pComp = rp.snapshot.modules.find(x => x.module === m);
      if (!pComp || !pComp.present || pComp.revision !== 1 || !eq(pComp.content, c1)) return { pass: false, detail: "published content mismatch: " + JSON.stringify(pComp).slice(0, 200) };
