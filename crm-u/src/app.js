(function () {
  ...
  const STORE_MODULES = ["companies", "contacts", "leads", "deals", "activities", "services", "sites", "assets", "tickets", "documents", "emails", "segments", "rules", "bus", "reports"];
  ...
  async function renderRoute(id, params) {
    ...
    document.title = `${def.label} · CRM-U`;
    try {
      const ctx = {
        id: def.id,
        def,
  ...
  function setupEnvPill() {
    ...
    pill.title = env.unsaved
      ...
      : "CRM-U workspace. Rename the generator in settings to rebrand.";
  }
