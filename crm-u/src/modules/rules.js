...
window.CRM_RULES = (function () {
  ...
  const FIELD_DEFS = {
    ...
    services: [
      ...
      { f: "category", label: "Category", type: "string" },
      { f: "charge", label: "Charge ($)", type: "num" },
      { f: "billingCycle", label: "Billing cycle", type: "string" },
      { f: "name", label: "Service name", type: "string" },
      { f: "mrr", label: "Monthly charge ($)", type: "num", scan: true },
      { f: "daysUntilRenewal", label: "Days until renewal", type: "num", scan: true },
      { f: "custom", label: "Custom field…", type: "custom" }
    ],
    sites: [
      { f: "siteType", label: "Site type", type: "string" },
      { f: "status", label: "Status", type: "string", options: ["active", "planned", "closed"] },
      { f: "name", label: "Site name", type: "string" },
      { f: "custom", label: "Custom field…", type: "custom" }
    ],
    assets: [
      { f: "kind", label: "Asset kind", type: "string" },
      { f: "status", label: "Status", type: "string" },
      { f: "identifier", label: "Identifier", type: "string" },
      { f: "name", label: "Asset name", type: "string" },
      { f: "warrantyEnd", label: "Warranty end", type: "date" },
      { f: "daysUntilWarranty", label: "Days until warranty ends", type: "num", scan: true },
      { f: "custom", label: "Custom field…", type: "custom" }
    ],
    tickets: [
      ...
      { f: "subject", label: "Subject", type: "string" },
      { f: "slaState", label: "SLA state", type: "string", scan: true },
      { f: "slaBreached", label: "SLA breached", type: "bool", scan: true },
      { f: "daysOpen", label: "Days open", type: "num", scan: true },
      { f: "custom", label: "Custom field…", type: "custom" }
    ]
  };
  ...
  const MODULE_LABELS = { deals: "Deals", leads: "Leads", companies: "Companies", contacts: "Contacts", services: "Services", sites: "Sites", assets: "Assets", tickets: "Tickets" };
  const RULE_MODULES = MODULES.map(m => m.id);
  ...
  function resolveRaw(rec, f, aidx, module) {
    ...
    if (!rec || rec.id === undefined) return undefined;
    const D = window.CRM_DOMAIN;
    if (module === "services") {
      if (f === "daysUntilRenewal") {
        const st = String(rec.status || "");
        if (st === "terminated" || st === "prospect" || st === "pending") return null;
        return D ? D.daysUntilRenewal(rec) : null;
      }
      if (f === "mrr") return D ? D.monthlyOf(rec) : money(rec.charge);
    }
    if (module === "assets" && f === "daysUntilWarranty") {
      return D && rec.warrantyEnd ? D.daysUntilDate(rec.warrantyEnd) : null;
    }
    if (module === "tickets") {
      if (f === "slaState") { const s = D ? D.ticketSlaState(rec) : null; return s ? s.state : undefined; }
      if (f === "slaBreached") {
        const s = D ? D.ticketSlaState(rec) : null;
        return !!(s && (s.state === "overdue" || s.state === "response-overdue" || s.state === "breached"));
      }
      if (f === "daysOpen") {
        const o = D ? D.parseMs(rec.openedAt || rec.createdAt) : null;
        return o ? (Date.now() - o) / DAY : null;
      }
    }
    return rec[f];
  }
  function money(v) {
    const n = Number(v);
    return v === "" || v === null || v === undefined || !isFinite(n) || n < 0 ? 0 : n;
  }
  ...
  function recName(rec, module) {
    ...
    if (module === "assets" && rec.identifier) return rec.identifier;
  ...
  async function resolveTemplate(tpl, store, module, rec) {
    ...
    let value = "";
    if (rec.expectedValue !== undefined && rec.expectedValue !== null) value = "$" + Number(rec.expectedValue).toLocaleString();
    else if (module === "services" && D) value = "$" + Math.round(D.monthlyOf(rec)).toLocaleString();
    const source = rec.source || "";
    const status = rec.status !== undefined ? String(rec.status) : "";
    const category = rec.category !== undefined ? String(rec.category) : (rec.siteType !== undefined ? String(rec.siteType) : "");
    let renewal = "";
    if (module === "services" && D) { const rd = D.renewalDateOf(rec); renewal = rd ? R.fmtDate(rd) : ""; }
    else if (module === "assets") renewal = rec.warrantyEnd ? R.fmtDate(rec.warrantyEnd) : "";
    else if (module === "tickets") renewal = rec.dueAt ? R.fmtDate(rec.dueAt) : "";
    else if (rec.closeDate) renewal = R.fmtDate(rec.closeDate);
    const date = R.fmtDate(localDateISO(new Date()));
    const vars = { name, owner, company, stage, value, source, date, status, category, renewal };
    return String(tpl || "").replace(/\{(name|owner|company|stage|value|source|date|status|category|renewal)\}/g, (m, k) => vars[k] || m);
  }
  ...
  async function handleWrite(store, data) {
    ...
    if (RULE_MODULES.indexOf(module) === -1) return;
  ...
  function actionRowView(rule, action, idx, onChanged) {
    ...
    function rebuild() {
      ...
      if (typeSel.value === "task") {
        ...
        inputs.appendChild(el("p", "hint", "Tokens: {name} {owner} {company} {stage} {value} {source} {status} {category} {renewal} {date}"));
