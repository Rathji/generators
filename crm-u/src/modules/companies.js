...
window.CRM_COMPANIES = (function () {
  ...
  async function renderDetail(ctx) {
    ...
    if (window.CRM_TICKETS && window.CRM_TICKETS.listCard) {
      try {
        const tkt = await window.CRM_TICKETS.listCard(store, { companyId: rec.id }, { title: "Open tickets", emptyText: "No open tickets for this account." });
        if (tkt) wrap.appendChild(tkt.el);
      } catch (e) {
        console.error("company tickets card failed:", e);
      }
    }

    if (window.CRM_DOCUMENTS && window.CRM_DOCUMENTS.card) {
      try {
        const dc = await window.CRM_DOCUMENTS.card(store, { ownerType: "company", ownerId: rec.id, title: "Account documents", emptyText: "No contracts or documents filed for this account yet." });
        if (dc) wrap.appendChild(dc.el);
      } catch (e) {
        console.error("company documents card failed:", e);
      }
    }
