...
window.CRM_REMINDERS = (function () {
  ...
  const WARRANTY_HORIZON_DAYS = 60;
  const SLA_SOON_MINUTES = 240;
  ...
  function serviceAlertable(s) {
    const st = String((s && s.status) || "");
    return st !== "terminated" && st !== "prospect" && st !== "pending";
  }

  async function alerts(store, opts) {
    opts = opts || {};
    const out = { renewals: [], sla: [], warranties: [], total: 0 };
    const D = window.CRM_DOMAIN;
    if (!D) return out;
    try {
      const doc = await store.loadDoc("services");
      for (const s of R.recordsOf(doc && doc.content)) {
        if (!serviceAlertable(s)) continue;
        const days = D.daysUntilRenewal(s);
        if (days === null || days > RENEWAL_HORIZON_DAYS) continue;
        out.renewals.push({ kind: "renewal", id: s.id, rec: s, due: D.renewalDateOf(s), days, overdue: days < 0, autoRenew: !!s.autoRenew, mrr: D.monthlyOf(s) });
      }
    } catch (e) {}
    try {
      const doc = await store.loadDoc("tickets");
      for (const t of R.recordsOf(doc && doc.content)) {
        const st = D.ticketSlaState(t);
        if (!st.open) continue;
        const mins = st.resolveMinutesLeft;
        const late = st.state === "overdue" || st.state === "response-overdue";
        const soon = st.state === "due-soon" || (mins !== null && mins <= SLA_SOON_MINUTES);
        if (!late && !soon) continue;
        out.sla.push({ kind: "sla", id: t.id, rec: t, due: (st.sla && st.sla.resolveDueAt) || t.openedAt || "", state: st.state, minutesLeft: mins, overdue: late });
      }
    } catch (e) {}
    try {
      const doc = await store.loadDoc("assets");
      for (const a of R.recordsOf(doc && doc.content)) {
        if (!a.warrantyEnd || a.status === "retired") continue;
        const days = D.daysUntilDate(a.warrantyEnd);
        if (days === null || days > WARRANTY_HORIZON_DAYS) continue;
        out.warranties.push({ kind: "warranty", id: a.id, rec: a, due: dateOnly(a.warrantyEnd), days, overdue: days < 0 });
      }
    } catch (e) {}
    const byDays = (x, y) => (x.overdue !== y.overdue ? (x.overdue ? -1 : 1) : x.days - y.days);
    out.renewals.sort(byDays);
    out.warranties.sort(byDays);
    out.sla.sort((x, y) => (x.overdue !== y.overdue ? (x.overdue ? -1 : 1) : (x.minutesLeft === null ? 1 : y.minutesLeft === null ? -1 : x.minutesLeft - y.minutesLeft)));
    out.total = out.renewals.length + out.sla.length + out.warranties.length;
    return out;
  }

  function alertTitle(it) {
    const rec = it.rec || {};
    if (it.kind === "renewal") return rec.name || rec.id;
    if (it.kind === "sla") return rec.subject || rec.id;
    if (it.kind === "warranty") return rec.name || rec.identifier || rec.id;
    return rec.id || "";
  }

  function alertHref(it) {
    const base = it.kind === "renewal" ? "services" : it.kind === "sla" ? "tickets" : "assets";
    return "#/" + base + "/" + encodeURIComponent(it.id);
  }
  ...
  function slaStateLabel(state) {
    if (state === "overdue") return "resolve target breached";
    if (state === "response-overdue") return "first response overdue";
    if (state === "due-soon") return "resolve target due soon";
    return "SLA on track";
  }

  function alertWhen(it) {
    if (it.kind === "sla") {
      if (it.state === "response-overdue") return "first response overdue";
      if (it.state === "overdue") return "resolve target overdue";
      const h = it.minutesLeft === null ? null : Math.max(0, Math.round(it.minutesLeft / 60));
      return h === null ? "due soon" : "~" + h + "h to resolve target";
    }
    return (it.overdue ? "overdue · " : "due ") + R.fmtDate(it.due) + (it.days !== undefined ? " · " + daysPhrase(it.days) : "");
  }

  async function alertCounts(store) {
    const a = await alerts(store);
    return { renewals: a.renewals.length, sla: a.sla.length, warranties: a.warranties.length, total: a.total };
  }

  function alertGroupNode(title, arr, maps) {
    ...
    const D = window.CRM_DOMAIN;
    const companies = (maps && maps.companies) || new Map();
    arr.forEach(it => {
      ...
      a.href = alertHref(it);
      a.textContent = alertTitle(it);
      t.appendChild(a);
      if (it.overdue) t.appendChild(el("span", "badge danger", "overdue"));
      main.appendChild(t);
      const bits = [];
      if (it.rec.companyId && companies.get(it.rec.companyId)) bits.push(companies.get(it.rec.companyId).name);
      if (it.kind === "renewal") bits.push((D ? D.categoryLabel(it.rec.category) : it.rec.category) + (it.autoRenew ? " · auto-renews" : ""));
      if (it.kind === "sla") bits.push(slaStateLabel(it.state) + (it.rec.assignee ? " · " + it.rec.assignee : " · unassigned"));
      if (it.kind === "warranty") bits.push(D ? D.assetKindLabel(it.rec.kind) : it.rec.kind || "");
      const sub = bits.filter(Boolean).join(" · ");
      if (sub) main.appendChild(el("div", "tl-sub", sub));
      row.appendChild(main);
      row.appendChild(el("span", "q-due" + (it.overdue ? " od" : ""), alertWhen(it)));
      sec.appendChild(row);
    });
    return sec;
  }
  ...
  async function renderMain(ctx, initialFilter) {
    ...
    hleft.appendChild(el("p", "hint", "Overdue and due-soon follow-ups plus deals closing within " + HORIZON_DAYS + " days, with a live panel of service renewals, ticket SLA and warranty alerts. Snooze, reschedule, reassign or dismiss — every action is written to the activity log."));
    ...
    async function paint() {
      ...
      const al = await alerts(store);
      const alertCard = el("section", "card rm-alerts");
      alertCard.dataset.rmAlerts = "1";
      const acr = el("div", "card-title-row");
      const acl = el("div");
      acl.appendChild(el("h2", null, "Service alerts"));
      acl.appendChild(el("p", "hint", "Renewals due within " + RENEWAL_HORIZON_DAYS + " days, tickets at risk of breaching SLA, and warranties ending soon — each links straight to the record."));
      acr.appendChild(acl);
      alertCard.appendChild(acr);
      const agroups = el("div", "digest-groups");
      if (!al.total) {
        agroups.appendChild(el("p", "hint muted-line", "No service alerts — renewals, SLAs and warranties are all current."));
      } else {
        agroups.appendChild(alertGroupNode("Renewals", al.renewals, maps));
        agroups.appendChild(alertGroupNode("Ticket SLA", al.sla, maps));
        agroups.appendChild(alertGroupNode("Warranties", al.warranties, maps));
      }
      alertCard.appendChild(agroups);
      body.innerHTML = "";
  ...
  async function digest(store) {
    ...
    const al = await alerts(store);
    out.renewals = al.renewals;
    out.sla = al.sla;
    out.warranties = al.warranties;
    out.alertCount = al.total;
    return out;
  }
  ...
  function digestText(d, maps) {
    ...
    sec("Renewals due in the next " + RENEWAL_HORIZON_DAYS + " days (" + renewals.length + ")");
    if (renewals.length) renewals.forEach(it => L.push("  " + alertTitle(it) + " — " + alertWhen(it) + (it.autoRenew ? " · auto-renews" : "") + (it.mrr ? " · $" + Math.round(it.mrr).toLocaleString() + "/mo" : "")));
    else L.push("  none");
    sec("SLA at risk or breached (" + sla.length + ")");
    if (sla.length) sla.forEach(it => L.push("  " + alertTitle(it) + " — " + alertWhen(it) + (it.rec.assignee ? " · " + it.rec.assignee : " · unassigned")));
    else L.push("  none");
    sec("Warranties ending in the next " + WARRANTY_HORIZON_DAYS + " days (" + warranties.length + ")");
    if (warranties.length) warranties.forEach(it => L.push("  " + alertTitle(it) + " — " + alertWhen(it)));
    else L.push("  none");
    L.push("");
    L.push("Total: " + (d.followupCount + d.closingCount + d.newLeads.length + renewals.length + sla.length + warranties.length) + " items need attention today.");
    return L.join("\n");
  }
  ...
  async function renderDigest(ctx) {
    ...
    groups.appendChild(alertGroupNode("Renewals due in the next " + RENEWAL_HORIZON_DAYS + " days", d.renewals || [], maps));
    groups.appendChild(alertGroupNode("SLA at risk or breached", d.sla || [], maps));
    groups.appendChild(alertGroupNode("Warranties ending in the next " + WARRANTY_HORIZON_DAYS + " days", d.warranties || [], maps));
    ...
    if (d.alertCount) summary.appendChild(el("span", "chip", d.alertCount + " service alerts"));
    ...
    card.appendChild(el("p", "hint", "The plain-text bundle above is the whole day at a glance — due follow-ups, closing deals, new leads, service renewals, ticket SLA and warranty alerts — ready to paste into a team channel."));
