/* ════════════════════════════════════════════════════════════════
   DOMINION COMPLETE — src/cards.js  (Task 2: schema + registry)
...
(function (global) {
  ...
  /* ── Reset + load a set list (browser path; uses fetch) ──
     ...
     repeatedly (tests do). A set with an in-development folder
     (src/exp/<set>/data.json exists) loads THAT catalog instead of
     src/data/<set>.json, and its implementations
     (src/exp/<set>/cards.js) are injected afterwards — so exp sets
     work exactly like shipped ones inside the harness while staying
     out of the shipped boot (which only requests installed sets).
     Non-browser contexts should use fs.readTextFile + loadFromData
     instead. */
  function injectScript(src) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      el.onerror = function () { reject(new Error("exp implementation failed to load: " + src)); };
