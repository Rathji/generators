...
(function () {
  ...
  let me = null; // {userId, displayName, role, func, financials, companies, boards, caps, admin}
  let peers = []; // [{userId, displayName, role, func, admin, focus}]
  ...
  let started = false;
  let modalOpen = false;
  let onMessageHook = null; // collab subscribes here for presence/change fan-out
  const syncTimers = {};
  ...
  /* Stable references: consumers (the collab service, tests) hold these, so we
     must never reassign the arrays — only mutate them in place. */
  Object.defineProperty(T, "peersList", { get() { return peers; } });
  Object.defineProperty(T, "hubIndex", { get() { return hubIndex; } });

  function active() { return (status === "online" && !store.backendOverridden()) || forceOnline; }
  T.active = active;

  /* Replace an array's contents in place (its identity is public). */
  function setList(arr, items) {
    arr.length = 0;
    (items || []).forEach((x) => arr.push(x));
    return arr;
  }

  /* The collaboration service subscribes to every hub message so it can drive
     presence chips and the active-station refresh. */
  T.setOnMessage = function (fn) { onMessageHook = typeof fn === "function" ? fn : null; };
  ...
  /* Ask the hub which change topics this user should watch: a user scoped to
     specific companies subscribes per company; an unrestricted user watches
     the global change topic. */
  function syncWatch() {
    if (!active() || !me) return Promise.resolve();
    const ids = (me.companies && me.companies.length) ? me.companies : [];
    return rpc("watch", { companies: ids }).then((r) => { T.watchResult = r; }).catch(() => {});
  }
  T.syncWatch = syncWatch;

  function applyHello(res) {
    me = {
      userId: res.userId, displayName: res.displayName, role: Number(res.role) || 0,
      func: res.func == null ? 4 : Number(res.func), financials: !!res.financials,
      companies: res.companies || [], boards: res.boards || [],
      caps: res.caps == null ? 0 : res.caps, admin: !!res.admin,
    };
    applyServerRole();
    if (ERP.collab && ERP.collab.onIdentity) ERP.collab.onIdentity();
  }
      ...
      .then((res) => {
        if (res && res.ok) {
          ...
          return rpc("index", {}).then((idx) => {
            if (idx && idx.ok) setList(hubIndex, idx.index || []);
          }).catch(() => {}).then(() => rpc("peers", {})).then((pl) => {
            if (pl && pl.ok) setList(peers, pl.peers || []);
          }).catch(() => {}).then(() => syncWatch());
        } else {
          setStatus("offline");
        }
      })
      .catch(() => setStatus("offline"))
      .then(() => { updateIndicator(); reRender(); if (ERP.collab && ERP.collab.refreshEditorChips) ERP.collab.refreshEditorChips(); });
  }
  ...
  function applyServerRole() {
    ...
    const chip = $("#roleChip");
    if (chip) chip.textContent = roleLabel(me.role);
    updateIndicator();
  }
  ...
  function scheduleSync(name) {
    ...
    syncTimers[name] = setTimeout(() => {
      ...
      store.syncDoc(name)
        .then((res) => {
          if (ERP.collab && ERP.collab.onDocSynced) ERP.collab.onDocSynced(name, res);
  ...
  function bumpIndex(d, r, u, n, ts) {
    ...
    if (hubIndex.length > 50) hubIndex.length = 50;
  }
  ...
  function onHubMessage(msg) {
    ...
    if (msg.t === "chg") {
      ...
      if (msg.on) {
        const prev = idx >= 0 ? peers[idx] : null;
        const rec = { userId: msg.u, displayName: msg.n || msg.u, role: msg.r, func: msg.f, admin: false, focus: prev ? prev.focus : null };
      ...
      const idx = peers.findIndex((p) => p.userId === msg.u);
      if (idx >= 0) peers[idx].focus = msg.on ? { kind: msg.kind, id: msg.id } : null;
    } else if (msg.t === "role") {
      if (me && msg.u === me.userId) {
        me.role = msg.r;
        if (msg.f != null) me.func = msg.f;
        if (active()) {
          applyServerRole();
          /* a role/scope change may alter which companies we may watch */
          whoami().then((w) => { if (w && w.ok) { applyHello(w); syncWatch(); } }).catch(() => {});
        }
      }
      const idx = peers.findIndex((p) => p.userId === msg.u);
      if (idx >= 0 && msg.f != null) peers[idx].func = msg.f;
      if (idx >= 0) peers[idx].role = msg.r;
    } else if (msg.t === "audit") {
      T.lastAuditSeq = msg.s;
    }
    if (onMessageHook) { try { onMessageHook(msg); } catch (e) {} }
    reRender();
  }
  ...
  /* Authorise a permission on the hub. `perm` is a permission key
     ("taxonomy.edit"); `ctx` may carry {companyId, boardId} so the hub can
     enforce the caller's record scope. Legacy guarded-action names still work
     because the hub maps them — but new callers should pass the permission.
     Throws a ready-to-show message when the hub refuses. */
  T.guard = async function (perm, ctx) {
    if (!active()) return;
    let res;
    try {
      res = await rpc("authorize", { perm: perm, ctx: ctx && (ctx.companyId != null || ctx.boardId != null) ? ctx : undefined });
    } catch (e) {
      throw new Error("The collaboration hub is unreachable — try again in a moment.");
    }
    if (res && res.ok) return;
    if (res && res.err === "rate") throw new Error("The collaboration hub is busy right now — try again in a moment.");
    throw new Error(guardMessage(perm, res));
  };

  function guardMessage(perm, res) {
    const C = ERP.collab;
    const who = C ? C.funcLabel(me ? me.func : 4) : "functional";
    if (res && res.reason === "capability") return "Your " + who + " role isn't allowed to do this on the hub — it needs " + ((res.requiredCap || "more") + " access") + ".";
    if (res && res.reason === "scope") return "This record is outside your " + (res.scope || "company") + " scope on the hub.";
    if (res && res.reason === "financial") return "Your account doesn't have financial visibility on the hub.";
    if (res && res.reason === "unknown") return "The hub doesn't recognise the permission \"" + perm + "\".";
    if (res && res.reason === "role") return "This action needs the " + (res.requiredRole || "owner") + " role on the hub (your server role: " + (me ? roleLabel(me.role) : "unknown") + ").";
    return "Not authorised by the collaboration hub" + (res && res.err ? " (" + res.err + ")" : "") + ".";
  }

  T.signAudit = async function (entry) {
    if (!active()) return null;
    const perm = entry.perm || (ERP.collab ? ERP.collab.permForAction(entry.action || "updated") : (entry.action || "updated"));
    const payload = {
      perm: perm,
    ...
    if (entry.ctx) payload.ctx = entry.ctx;
    let res;
    try { res = await rpc("approve", payload); } catch (e) { return null; }
    if (!res) return null;
    if (res.ok && res.entry) return { entry: res.entry };
    if (res.denied) return { denied: res.reason2 || res.reason || "not authorised" };
  ...
  /* Full access patch: {role, func, financials, companies, boards}. */
  T.setUser = async function (userId, patch) {
    const res = await rpc("setUser", Object.assign({ userId: userId }, patch || {}));
    return res || { ok: false, err: "no reply" };
  };
  ...
  T.peers = async function () {
    const r = await rpc("peers", {});
    if (r && r.ok) setList(peers, r.peers || []);
    if (ERP.collab && ERP.collab.refreshEditorChips) ERP.collab.refreshEditorChips();
    return r || { ok: false };
  };
  ...
  T.rateStats = async function () { return await rpc("rateStats", {}); };
  T.authorize = async function (perm, ctx) { return await rpc("authorize", { perm: perm, ctx: ctx || undefined }); };
  T.watch = async function (companies) { return await rpc("watch", { companies: companies || [] }); };
  /* Editing presence: tell the hub which record this session is editing. */
  T.focus = async function (kind, id, on) { return await rpc("focus", { kind: kind, id: id, on: !!on }); };
