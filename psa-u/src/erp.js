/* ============================================================
   PSA-U — application shell & module framework
   (inherited from Business ERP; internal `ERP` namespace kept as-is)
...
(function () {
  ...
  const ICONS = {
    ...
    agreement: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z"/><path d="M14 2v6h6"/><path d="M8 13h8"/><path d="M8 17h5"/>',
  ...
  /* Tab panels (ui.tabs) carry erp-tab-panel[/active]; the state renderers must
     not clobber that, or every visited tab stays visible. Everything else is
     reset to the standard erp-content layout class. */
  function stateClass(el) {
    return /(^|\s)erp-tab-panel(\s|$)/.test(el.className || "") ? el.className : "erp-content";
  }

  ERP.states = {
    loading(el, label) {
      el.className = stateClass(el);
      el.innerHTML =
  ...
  const GROUPS = [
    { key: null, label: null },
    { key: "client", label: "Client & pipeline" },
    { key: "delivery", label: "Delivery & resourcing" },
    { key: "fin", label: "Billing & finance" },
  ];
  ...
  function navGroups() {
    return ERP.groups && ERP.groups.length ? ERP.groups : GROUPS;
  }
  ...
  function setTitle(module) {
    ...
    document.title = (module ? module.label + " · " : "") + configVal("branding.companyName", "PSA-U");
  }
  ...
  function boot() {
    ...
    if (brand) brand.textContent = configVal("branding.companyName", "PSA-U");
    const mark = $("#logoMark");
    if (mark) mark.textContent = configVal("branding.logoGlyph", "P").slice(0, 2);

    _role = (() => {
      try { const s = localStorage.getItem(LS.role); if (ERP.ROLES.indexOf(s) !== -1) return s; } catch (e) {}
      return configVal("psa.roleDefault", DEFAULT_ROLE);
    })();
    ...
    wire();
    bootProgress();
    // Module scripts (src/psa.modules.js) execute right after this one; wait
