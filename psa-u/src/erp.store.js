/* ============================================================
   ...
   other side changed a document. The store index (psa-v1-index)
...
(function () {
  ...
  const STORE = {
    namespace: "psa",           // editable-name prefix: psa-v1-<doc>[-<year>]
    schemaVersion: 1,
  ...
  const LS = {
    index: "psa.store.v1.index",      // cached index JSON
    cache: "psa.store.v1.cache.",     // + docName → cached doc JSON
    keys: "psa.store.v1.keys.",       // + docName → edit key
    alias: "psa.store.v1.alias.",     // + docName → recovered editable name (edit key lost)
    flushes: "psa.store.v1.flushes",  // doc names with pending canonical writes
    base: "psa.store.v1.base.",       // + docName → {rev, records|null} last-synced watermark (Task 3)
    conflicts: "psa.store.v1.conflicts", // JSON array of pending conflicts (Task 3)
    merge: "psa.store.v1.merge.",     // + docName → in-progress field-level merge state (Task 3)
  };
    ...
    try {
      ...
      if (res.error === "edit_key_required" || res.error === "invalid_edit_key") {
        /* Self-heal: the canonical file exists but we no longer hold its
           ...
           or the cached key is stale/wrong). Adopt a fresh editable name
           for this doc and remember the alias so every later read/write
           routes through it; the orphaned file is abandoned. */
  ...
  /* Given a full store name, return the logical document name for the
     envelope (e.g. "psa-v1-crm" → "crm" when a declared module matches). */
  ...
  /* Fiscal year of a record date. January-start by default; reads
     config.psa.fiscalYearStartMonth (1=Jan) once fiscal settings
  ...
  function fiscalYearOf(record) {
    ...
    let startMonth = 1;
    try {
      const m = parseInt(ERP.configVal("psa.fiscalYearStartMonth", 1), 10);
  ...
  /* Commit notification: fired after a document's content is durably committed
     (locally + canonical). The version-history service subscribes to keep a
     restorable revision log for every document. Purely additive — nothing in
     the store depends on a listener being present. */
  let commitListener = null;
  function setCommitListener(fn) { commitListener = typeof fn === "function" ? fn : null; }
  function notifyCommit(name, doc) {
    if (commitListener) { try { commitListener(name, doc); } catch (e) {} }
  }
  ...
  function refreshDoc(name) {
    ...
    editableGet(name).then((res) => {
      ...
      if (!remote || remote.schema !== "psa-doc") return;
      const local = cacheGet(name);
      if (!local) return;
  ...
  function loadIndexFromCache() {
    ...
    try {
      ...
      return d && d.schema === "psa-index" ? d : null;
    } catch (e) { return null; }
  }
  ...
  async function writeIndex() {
    let idx = loadIndexFromCache();
    if (!idx) {
      /* The local cache can legitimately be absent (tests / restore / a fresh
         device). In that case adopt the canonical index as the base rather
         than inventing an empty one — otherwise a late debounced write would
         clobber the canonical index with {}. */
      const res = await editableGet(indexName());
      if (!res.error && res.text != null) {
        try { const parsed = JSON.parse(res.text); if (parsed && parsed.schema === "psa-index") idx = parsed; } catch (e) {}
      }
  ...
  async function getIndex(opts) {
    ...
    if (idx.documents && typeof idx.documents === "object") {
      /* Merge in any document this device knows about but the canonical index
         has not caught up with yet (its write is debounced), so a force-read
         never drops a locally-known document from the index. */
      const cached = loadIndexFromCache();
      if (cached && cached.documents) {
        for (const k in cached.documents) if (!idx.documents[k]) idx.documents[k] = cached.documents[k];
      }
      writeLS(LS.index, serialize(idx));
  ...
  const api = {
    ...
    /* Generic write of any document by its full store name. Accepts a
       ...
       which gets wrapped in a versioned psa-doc envelope. */
    async set(name, recordsOrDoc) {
      let doc = recordsOrDoc;
      if (!doc || doc.schema !== "psa-doc") {
        doc = {
          schema: "psa-doc",
          schemaVersion: STORE.schemaVersion,
    ...
    async readCanonical(name) {
      ...
      if (doc && doc.schema === "psa-doc") cachePut(name, doc);
      return { doc: doc && doc.schema === "psa-doc" ? doc : null };
    },
    ...
    /* Rebuild the store index from the canonical documents — healing for a
       lost or emptied index. Scans every declared module document plus every
       document this device has cached, records each canonical copy it finds,
       and writes the merged index back. */
    async reindex() {
      const names = new Set();
      for (const d of declaredDocs()) names.add(docName(d.module));
      for (const n of cachedDocNames()) names.add(n);
      const existing = loadIndexFromCache() || emptyIndex();
      existing.documents = existing.documents || {};
      for (const name of names) {
        if (name === indexName() || existing.documents[name]) continue;
        const r = await editableGet(name);
        if (r.error || r.text == null) continue;
        let doc;
        try { doc = JSON.parse(r.text); } catch (e) { continue; }
        if (!doc || doc.schema !== "psa-doc") continue;
        cachePut(name, doc);
        touchIndexFromDoc(name, doc);
      }
      await writeIndex();
      return getIndex({ force: true });
    },
