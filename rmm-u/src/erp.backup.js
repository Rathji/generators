/* ============================================================
   ...
   UI lives under Reports → Data utilities (erp.reporting.js hosts
   the tab; this file exposes ERP.backup.{engine, renderPanel}).
   ============================================================ */
...
(function () {
  ...
  const BACKUP_NAME = "rmm-v1-backup";
  ...
  backup.backupBundle = async function () {
    ...
    return {
      schema: "rmm-backup",
      version: 1,
      generator: window.generatorName || "rmm-u",
  ...
  backup.downloadBackup = async function () {
    ...
    a.download = (window.generatorName || "rmm-u") + "-backup-" + new Date().toISOString().slice(0, 10) + ".json";
  ...
  /* Publish the bundle to RMM-U's own namespace. Re-publishing when no
  ...
  backup.publishBackup = async function () {
    if (ERP.team && typeof ERP.team.guard === "function") await ERP.team.guard("publish_backup");
    const bundle = await backup.backupBundle();
  ...
  backup.publishedBackup = async function () {
    ...
    if (r.doc && r.doc.records && r.doc.records[0] && r.doc.records[0].schema === "rmm-backup") return r.doc.records[0];
    return null;
  };

  /* Validate a bundle. Returns { valid, errors: [{doc, error}], docs }.
     A document is only included if it parses as a versioned rmm-doc. */
  backup.validateBundle = function (obj) {
    const errors = [];
    if (!obj || obj.schema !== "rmm-backup") {
      return { valid: false, errors: [{ doc: "(bundle)", error: "Not an RMM-U backup — expected schema \"rmm-backup\"." }], docs: {} };
    }
    const docs = {};
    for (const name of Object.keys(obj.docs || {})) {
      const d = obj.docs[name];
      if (!d || d.schema !== store.docSchema) {
        errors.push({ doc: name, error: "Skipped: not a versioned rmm-doc." });
        continue;
      }
  ...
  backup.restoreBundle = async function (bundle, opts) {
    if (ERP.team && typeof ERP.team.guard === "function") await ERP.team.guard("restore_backup");
    opts = opts || {};
  ...
  backup.archiveDoc = async function (moduleId, year) {
    if (ERP.team && typeof ERP.team.guard === "function") await ERP.team.guard("archive_doc");
    ...
    await ERP.master.audit({ action: "archive", targetType: "doc", targetId: moduleId + ":" + year, summary: "Archived " + records.length + " record(s) from " + mod.label + " FY " + year + " to the read-only archive." });
  ...
  backup.restoreArchive = async function (archiveId) {
    ...
    const w = await store.forceSet(name, {
      schema: store.docSchema, schemaVersion: 1, doc: mod.doc.name, year: entry.originYear, rev: 0,
    ...
    const render = async () => {
      ...
      pane("backup").innerHTML =
        ui.card("Full backup", lastPub +
          "<p class=\"erp-alert\">Download saves every module document to a JSON file on your device. Publish stores the same bundle in RMM-U's own storage namespace (re-publishing identical content is a safe no-op).</p>" +
      ...
      ui.bind(el, "click", "[data-act]", async (t, e, act) => {
        if (act === "download-backup") {
          ...
          try {
            ...
            ERP.toast(res.noop ? "Backup already published (identical content)." : "Backup published to the RMM-U namespace.", "success");
            render();
  ...
  backup.archiveViewUi = function (entry, rerender) {
    ...
    const table = entry.records && entry.records.length
      ? ui.table(cols.map((k) => ({ key: k, label: k })), entry.records, { scroll: true, emptyText: "No records." })
