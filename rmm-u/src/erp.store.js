/* ============================================================
   RMM-U — canonical document store (Task 2)
   Every document persists as versioned JSON in RMM-U's own storage
   namespace. A document is either a module document (one per
   console data module) or, for tenancy, one document per managed-
   service provider (rmm-v1-provider-<id>) plus the provider
   registry (rmm-v1-providers) — see src/rmm.tenancy.js:
     - canonical copy: upload-plugin editable files keyed to this
       generator, so records survive across devices AND reloads.
       Module documents may be split by fiscal year.
   ...
   is what sync/conflict handling uses to detect that the other side
   changed a document. The store index (rmm-v1-index) tracks the doc
   name, year, rev, record count, byte size and timestamp of every
   document — the basis for sync and for capacity & archival.
...
(function () {
  ...
  const STORE = {
    // Editable-name prefix: rmm-v1-<doc>[-<year>]. Read from config so the
    // platform always owns its own namespace (config.rmm.namespace).
    namespace: (function () {
      try {
        const n = ERP.configVal("rmm.namespace", "rmm");
        return n ? String(n) : "rmm";
      } catch (e) { return "rmm"; }
    })(),
    docSchema: "rmm-doc",       // versioned-document envelope schema
    indexSchema: "rmm-index",   // store index schema
    schemaVersion: 1,
  ...
  const LS = {
    index: "rmm.store.v1.index",      // cached index JSON
    cache: "rmm.store.v1.cache.",     // + docName → cached doc JSON
    keys: "rmm.store.v1.keys.",       // + docName → edit key
    alias: "rmm.store.v1.alias.",     // + docName → recovered editable name (edit key lost)
    flushes: "rmm.store.v1.flushes",  // doc names with pending canonical writes
    base: "rmm.store.v1.base.",       // + docName → {rev, records|null} last-synced watermark (Task 3)
    conflicts: "rmm.store.v1.conflicts", // JSON array of pending conflicts (Task 3)
    merge: "rmm.store.v1.merge.",     // + docName → in-progress field-level merge state (Task 3)
  };
  ...
  /* Given a full store name, return the logical document name for the
     envelope (e.g. "rmm-v1-crm" → "crm" when a declared module matches). */
  ...
  function refreshDoc(name) {
    ...
    editableGet(name).then((res) => {
      ...
      if (!remote || remote.schema !== STORE.docSchema) return;
      const local = cacheGet(name);
      if (!local) return;
  ...
  function loadIndexFromCache() {
    ...
    try {
      ...
      return d && d.schema === STORE.indexSchema ? d : null;
    } catch (e) { return null; }
  }
  ...
  const api = {
    ...
    docSchema: STORE.docSchema,
    indexSchema: STORE.indexSchema,
    schemaVersion: STORE.schemaVersion,
    ...
    /* Generic compare-and-set write of any document by its full store
       name. Accepts a full document object (has .schema) or a plain
       records array, which gets wrapped in a versioned envelope. `meta`
       (optional) is merged into the store-index entry for the doc — pass
       {module, name, year} so a non-module doc such as a provider
       aggregate is labelled correctly. `opts.force` bypasses the
       compare-and-set conflict check (see forceSet). */
    async put(name, recordsOrDoc, meta, opts) {
      let doc = recordsOrDoc;
      if (!doc || doc.schema !== STORE.docSchema) {
        doc = {
          schema: STORE.docSchema,
          schemaVersion: STORE.schemaVersion,
      ...
      return writeNamedDoc(name, doc, meta, opts);
    },
    async set(name, recordsOrDoc) { return api.put(name, recordsOrDoc); },
    ...
    async readCanonical(name) {
      ...
      if (doc && doc.schema === STORE.docSchema) cachePut(name, doc);
      return { doc: doc && doc.schema === STORE.docSchema ? doc : null };
    },
    ...
    resetLocal(moduleId) {
      if (moduleId) {
        ...
        const aprefix = LS.alias + STORE.namespace + "-v" + STORE.schemaVersion + "-" + d.name;
        try {
          for (let i = localStorage.length - 1; i >= 0; i--) {
            const k = localStorage.key(i);
            if (k && (k.indexOf(prefix) === 0 || k.indexOf(kprefix) === 0 || k.indexOf(bprefix) === 0 || k.indexOf(mprefix) === 0 || k.indexOf(aprefix) === 0)) localStorage.removeItem(k);
          }
      ...
      try {
        for (let i = localStorage.length - 1; i >= 0; i--) {
          ...
          if (k && k.indexOf(LS.alias) === 0) localStorage.removeItem(k);
