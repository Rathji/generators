...
(function () {
  ...
  ui.table = function (cols, rows, opts) {
    ...
    const body = rows.length
      ? rows.map((r) => "<tr>" + cols.map((c) => '<td class="' + (c.align ? "align-" + c.align : "") + '">' + (c.render ? c.render(r) : r[c.key] != null ? esc(String(r[c.key])) : "") + "</td>").join("") + "</tr>").join("")
  ...
  ui.text = function (name, label, val, ph) {
    return ui.field(label, '<input type="text" name="' + esc(name) + '" value="' + esc(val == null ? "" : val) + '" placeholder="' + esc(ph || "") + '">');
  };
  ui.number = function (name, label, val, o) {
    o = o || {};
    const step = o.step == null ? "any" : o.step;
    return ui.field(label, '<input type="number" step="' + esc(step) + '" name="' + esc(name) + '" value="' + (val == null ? "" : esc(val)) + '"' + (o.min != null ? ' min="' + esc(o.min) + '"' : "") + '>', o.hint);
  };
  ui.select = function (name, label, opts, val, ph) {
    ...
    return ui.field(label, '<select name="' + esc(name) + '">' + (ph ? '<option value="">' + esc(ph) + "</option>" : "") + options + "</select>");
  };
  ui.textarea = function (name, label, val, rows) {
    return ui.field(label, '<textarea name="' + esc(name) + '" rows="' + (rows || 3) + '">' + esc(val || "") + "</textarea>");
  };
  ui.dateInput = function (name, label, val) {
    return ui.field(label, '<input type="date" name="' + esc(name) + '" value="' + esc(val || "") + '">');
  };
