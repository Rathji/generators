...
(function () {
  ...
  let viewersByDevice = {}; // deviceId -> [{userId, displayName, role, admin, at}] (Task 49)
  let lastConflict = null; // {name, mine, theirs, by, ts} (Task 50)
  ...
  function onHubMessage(msg) {
    ...
    if (msg.t === "chg") {
      ...
      viewersByDevice[msg.d] = Array.isArray(msg.users) ? msg.users : [];
      viewerListeners.slice().forEach((fn) => { try { fn(msg.d, viewersByDevice[msg.d]); } catch (e) {} });
    } else if (msg.t === "conflict") {
      lastConflict = { name: msg.d, mine: msg.mine, theirs: msg.theirs, by: msg.n || msg.u || "a teammate", ts: msg.ts, at: new Date().toISOString() };
      T.lastConflict = lastConflict;
      conflictListeners.slice().forEach((fn) => { try { fn(lastConflict); } catch (e) {} });
      if (active()) ERP.toast("Another user saved a newer version of " + logicalName(msg.d) + " — review it in Sync & conflicts.", "error");
    } else if (msg.t === "audit") {
  ...
  /* RMM role & access (Task 48) — the console's window onto the collector's
     access registry. The hub re-checks `access.manage` on every call. */
  T.rmmCatalog = async function () { return await rpc("rmmCatalog", {}); };
  T.rmmAuthorize = async function (opts) { return await rpc("rmmAuthorize", opts || {}); };
  T.rmmListAccess = async function () { return await rpc("rmmListAccess", {}); };
  T.rmmSetRole = async function (userId, role) { return await rpc("rmmSetRole", { userId: userId, role: role }); };
  T.rmmSetScopes = async function (userId, scopes) { return await rpc("rmmSetScopes", { userId: userId, scopes: scopes }); };
  T.rmmSetCaps = async function (userId, allow, deny) { return await rpc("rmmSetCaps", { userId: userId, allow: allow, deny: deny }); };
  T.rmmRemoveAccess = async function (userId) { return await rpc("rmmRemoveAccess", { userId: userId }); };

  /* Realtime console (Task 49) — console presence & viewer fan-out. */
  T.rmmFocus = async function (deviceId, providerId) { return await rpc("rmmFocus", { deviceId: deviceId, providerId: providerId }); };
  T.rmmViewers = async function (deviceId) { return await rpc("rmmViewers", { deviceId: deviceId }); };
  T.rmmPresenceList = async function () { return await rpc("rmmPresenceList", {}); };
  T.viewers = function (deviceId) { return (viewersByDevice[deviceId] || []).slice(); };
  T.viewersByDevice = viewersByDevice;
  T.onViewers = function (fn) {
    if (typeof fn !== "function") return () => {};
    viewerListeners.push(fn);
    return () => { const i = viewerListeners.indexOf(fn); if (i >= 0) viewerListeners.splice(i, 1); };
  };
  T.lastConflict = null;

  /* Multi-user audit & conflict safety (Task 50). */
  T.rmmConnStats = async function () { return await rpc("rmmConnStats", {}); };
  T.rmmClaim = async function (doc, baseRev) { return await rpc("rmmClaim", { doc: doc, baseRev: baseRev }); };
  T.rmmRelease = async function (doc) { return await rpc("rmmRelease", { doc: doc }); };
  T.rmmClaims = async function () { return await rpc("rmmClaims", {}); };
  T.rmmAudit = async function (action, opts) { return await rpc("rmmAudit", Object.assign({ action: action }, opts || {})); };
  T.onConflict = function (fn) {
    if (typeof fn !== "function") return () => {};
    conflictListeners.push(fn);
    return () => { const i = conflictListeners.indexOf(fn); if (i >= 0) conflictListeners.splice(i, 1); };
  };
