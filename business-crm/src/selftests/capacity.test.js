(function () {
  ...
  function mockEnv(opts) {
    ...
    const ns = opts.ns || "ca" + BS.randHex(6);
    const modules = opts.modules || BS.DEFAULT_MODULES.slice();
  ...
  function dealRec(id, title, closedAt) {
    return { id, title, value: 1000 * id, closedAt };
  }

  function oldDeals() {
    return [dealRec(1, "Old Corp A", isoYearsAgo(2, -60000)), dealRec(2, "Old Corp B", isoYearsAgo(2, -120000))];
  }

  function freshDeals() {
    return [dealRec(3, "Fresh A", isoYearsAgo(0, -30 * 86400000)), dealRec(4, "Fresh B", isoYearsAgo(0, -20 * 86400000))];
  }

  T.register("capacity: capacityInfo sizes live docs, files and archives for every module", async () => {
    const env = mockEnv({ modules: ["companies", "deals"] });
    const c1 = { records: [{ id: 1, name: "Acme" }] };
    await seedDoc(env.store, "companies", c1);
    ...
    const comp = info.modules.companies;
    if (!comp.present || comp.revision !== 1 || comp.records !== 1 || comp.parts !== 1 || comp.liveBytes <= 0 || comp.maxFileBytes !== comp.liveBytes) {
      return { pass: false, detail: "companies row wrong: " + JSON.stringify(comp).slice(0, 220) };
    }
    const dl = info.modules.deals;
    if (dl.present || dl.state !== "none" || dl.liveBytes !== 0 || dl.archive) return { pass: false, detail: "empty module row wrong: " + JSON.stringify(dl).slice(0, 220) };
    ...
    const h = headOf(env, env.store, "companies");
    if (!h || h.revision !== 1) return { pass: false, detail: "seed missing from files" };
    if (comp.liveBytes !== new TextEncoder().encode(env.files.get(env.store.fileName("companies")).text).length) {
      return { pass: false, detail: "reported bytes do not match the actual file" };
    }
    return { pass: true, detail: "seeded module measured (rev 1, 1 record, 1 file, bytes match the real file); empty module reported present:false" };
  });

  T.register("capacity: a multi-part live document is reported with every file and summed bytes", async () => {
    const env = mockEnv({ modules: ["deals"], budget: 4000 });
    const pad = "x".repeat(1500);
    const recs = [];
    for (let i = 1; i <= 4; i++) recs.push({ id: i, title: "Big deal " + i, pad, closedAt: isoYearsAgo(1) });
    await seedDoc(env.store, "deals", { records: recs });
    const info = await env.store.capacityInfo();
    const row = info.modules.deals;
  ...
  T.register("capacity: previewArchive matches only records older than the cutoff, and guards its inputs", async () => {
    const env = mockEnv({ modules: ["deals"] });
    const recs = oldDeals().concat(freshDeals());
    await seedDoc(env.store, "deals", { records: recs });
    const cutoff = new Date();
    cutoff.setFullYear(cutoff.getFullYear() - 1);
    const pv = await env.store.previewArchive("deals", { field: "closedAt", before: cutoff.toISOString() });
    ...
    if (pv.bytes <= 0 || pv.field !== "closedAt" || !pv.before) return { pass: false, detail: "preview shape wrong" };
    const none = await env.store.previewArchive("deals", { field: "stage", before: cutoff.toISOString() });
    if (!none.ok || none.count !== 0) return { pass: false, detail: "a non-date field should match nothing" };
    const badField = await env.store.previewArchive("deals", { before: cutoff.toISOString() });
    if (badField.ok || badField.code !== "field_required") return { pass: false, detail: "missing field not refused" };
    const badBefore = await env.store.previewArchive("deals", { field: "closedAt", before: "not-a-date" });
    if (badBefore.ok || badBefore.code !== "bad_before") return { pass: false, detail: "bad cutoff not refused" };
    const badMod = await env.store.previewArchive("widgets", { field: "closedAt", before: cutoff.toISOString() });
    if (badMod.ok || badMod.code !== "bad_module") return { pass: false, detail: "unknown module not refused" };
    const nested = await env.store.previewArchive("deals", { field: "meta.won_at", before: new Date().toISOString() });
    if (!nested.ok || nested.count !== 0) return { pass: false, detail: "nested path on records without it should match nothing" };
    const h = headOf(env, env.store, "deals");
  ...
  T.register("capacity: archiveRecords writes a read-only batch, prunes the live doc and never duplicates a batch", async () => {
    ...
    const ar = await env.store.archiveRecords("deals", { field: "closedAt", before: beforeISO });
    ...
    const la = await env.store.listArchive("deals");
    ...
    if (b.no !== 1 || b.count !== 2 || b.rule.field !== "closedAt" || b.status !== "archived" || b.restoredAt !== null || !b.readOnly) {
      return { pass: false, detail: "index entry wrong: " + JSON.stringify(b) };
    }
    if (!env.files.has(env.store.archiveBatchName("deals", 1))) return { pass: false, detail: "batch file not written" };
    const again = await env.store.archiveRecords("deals", { field: "closedAt", before: beforeISO });
    if (!again.ok || again.archived !== 0) return { pass: false, detail: "second archive should find nothing: " + JSON.stringify(again) };
    const la2 = await env.store.listArchive("deals");
  ...
  T.register("capacity: a large batch splits across part files and reads back whole", async () => {
    ...
    const recs = [];
    for (let i = 1; i <= 6; i++) recs.push({ id: i, title: "Ancient " + i, pad, closedAt: isoYearsAgo(3) });
    await seedDoc(env.store, "deals", { records: recs });
    const cutoff = new Date();
    cutoff.setFullYear(cutoff.getFullYear() - 1);
    const ar = await env.store.archiveRecords("deals", { field: "closedAt", before: cutoff.toISOString() });
    ...
    const rb = await env.store.readArchiveBatch("deals", 1);
  ...
  T.register("capacity: a corrupt archive index blocks archiving and leaves live data untouched", async () => {
    ...
    const recs = oldDeals().concat(freshDeals());
    await seedDoc(env.store, "deals", { records: recs });
    const idxName = env.store.archiveIndexName("deals");
    const cutoff = new Date();
    cutoff.setFullYear(cutoff.getFullYear() - 1);
    const ar0 = await env.store.archiveRecords("deals", { field: "closedAt", before: cutoff.toISOString() });
    ...
    const wouldMatch = await env.store.previewArchive("deals", { field: "closedAt", before: later });
    ...
    const ar = await env.store.archiveRecords("deals", { field: "closedAt", before: later });
  ...
  T.register("capacity: a device without write keys archives safely but cannot prune, and retries reuse the batch", async () => {
    ...
    const ar = await dev.store.archiveRecords("deals", { field: "closedAt", before: cutoff.toISOString() });
    ...
    const la = await dev.store.listArchive("deals");
    if (!la.ok || la.batches.length !== 1) return { pass: false, detail: "archive index entry missing despite failed prune" };
    if (!env.files.has(env.store.archiveBatchName("deals", 1))) return { pass: false, detail: "batch file missing despite failed prune" };
    const h = headOf(env, env.store, "deals");
    if (h.revision !== 1 || h.content.records.length !== 2) return { pass: false, detail: "live data should be untouched by a keyless device" };
    const ar2 = await dev.store.archiveRecords("deals", { field: "closedAt", before: cutoff.toISOString() });
    if (ar2.ok || ar2.code !== "no_edit_key") return { pass: false, detail: "second attempt wrong: " + JSON.stringify(ar2).slice(0, 200) };
    const la2 = await dev.store.listArchive("deals");
    if (la2.batches.length !== 1 || env.files.has(env.store.archiveBatchName("deals", 2))) {
  ...
  T.register("capacity: restoreArchiveBatch merges records back into live data and marks the batch restored", async () => {
    ...
    const rr = await env.store.restoreArchiveBatch("deals", 1);
    ...
    const h3 = headOf(env, env.store, "deals");
    ...
    const rb2 = await env.store.readArchiveBatch("deals", 2);
    if (!rb2.ok || rb2.records.length !== 2) return { pass: false, detail: "batch b2 unreadable" };
    const missing = await env.store.restoreArchiveBatch("deals", 9);
  ...
  T.register("capacity: reading a batch fails cleanly when a batch file was removed", async () => {
    const env = mockEnv({ modules: ["deals"] });
    await seedDoc(env.store, "deals", { records: oldDeals() });
    const cutoff = new Date();
    cutoff.setFullYear(cutoff.getFullYear() - 1);
    const ar = await env.store.archiveRecords("deals", { field: "closedAt", before: cutoff.toISOString() });
    if (!ar.ok) return { pass: false, detail: "archive failed" };
    env.files.delete(env.store.archiveBatchName("deals", 1));
    const rb = await env.store.readArchiveBatch("deals", 1);
  ...
  T.register("capacity: capacity zone renders and archives records through the UI flow", async () => {
    if (!window.CRM_CAPACITY || !window.CRM_CAPACITY.renderZone) return { pass: true, skip: true, detail: "capacity UI not loaded" };
    const env = mockEnv({ modules: ["companies"] });
    const recs = [];
    const old = [{ id: 1, name: "Dusty Co" }, { id: 2, name: "Musty Ltd" }];
    const fresh = [{ id: 3, name: "Active Inc" }, { id: 4, name: "New Corp" }];
    for (const r of old) recs.push({ id: r.id, name: r.name, industry: "retail", closedAt: isoYearsAgo(2, -90000) });
    for (const r of fresh) recs.push({ id: r.id, name: r.name, industry: "retail", closedAt: isoYearsAgo(0, -15 * 86400000) });
    await seedDoc(env.store, "companies", { records: recs });
    const holder = document.createElement("div");
    holder.style.position = "absolute";
    holder.style.left = "-10000px";
    document.body.appendChild(holder);
    try {
      const zone = await window.CRM_CAPACITY.renderZone(env.store);
      ...
      modSel.value = "companies";
      const fieldReady = await until(() => {
        return Array.from(fieldSel.options).some(o => o.value === "closedAt") ? "closedAt" : null;
      }, 6000, 120);
      if (!fieldReady) return { pass: false, detail: "closedAt never appeared in the field picker" };
      previewBtn.click();
      ...
      const la = await env.store.listArchive("companies");
      ...
      const rb = await env.store.readArchiveBatch("companies", 1);
  ...
  T.register("capacity: real archive → restore round-trip on live server files (needs saved generator)", async () => {
    ...
    const ns = "cav" + Date.now().toString(36) + BS.randHex(4);
    let store;
    try {
      store = BS.create({
        ns,
    ...
    const seed = oldDeals().concat(freshDeals());
    ...
    try {
      ...
      const cutoff = new Date();
      cutoff.setFullYear(cutoff.getFullYear() - 1);
      const pv = await store.previewArchive(m, { field: "closedAt", before: cutoff.toISOString() });
      if (!pv.ok || pv.count !== 2) return { pass: false, detail: "live preview wrong: " + JSON.stringify(pv).slice(0, 200) };
      const tA = Date.now();
      const ar = await store.archiveRecords(m, { field: "closedAt", before: cutoff.toISOString() });
      ...
      return { pass: true, detail: "seeded 4 deals, archived the 2 closed 2y ago into b1, live pruned to rev 2, batch read back read-only, restored to rev 3 and marked restored (" + Math.round((Date.now() - t0) / 1000) + "s incl. write throttles)" };
    } finally {
      await cleanup();
    }
  });
