window.CRM_BACKUP = (function () {
  ...
  function recCount(content) {
    return Array.isArray(content && content.records) ? content.records.length : 0;
  }
  function fmtBytes(n) {
    if (n == null) return "";
  ...
  function backupFilename(snap) {
    ...
    return "business-crm-backup-" + stamp + ".json";
  }
  ...
  function statusShow(statusEl, kind, text) {
    ...
    const m = el("div", "bkp-msg " + kind, text);
  ...
  async function openRestoreFlow(zone, store, sourceText, sourceLabel) {
    ...
    let val = await store.validateBackup(sourceText).catch(e => ({ ok: false, code: "error", detail: (e && e.message) || String(e) }));
    if (!val.ok) {
    ...
    const noopRows = preview.rows.filter(r => r.action === "noop");
    ...
    const hasData = preview.rows.some(r => r.present && r.action !== "noop");
    if (!writeRows.length) {
      const m = el("div", "bkp-msg ok");
      m.textContent = "Every module in this backup already matches today's data — restoring would change nothing.";
      banners.appendChild(m);
    }
    ...
    for (const r of preview.rows) {
      ...
      if (r.present) snapTd.appendChild(el("span", "bkp-cell", "rev " + r.snapRev + " · " + r.records + " rec"));
      else snapTd.appendChild(el("span", "bkp-cell", "no data"));
    ...
    restoreBtn.addEventListener("click", async () => {
      ...
      if (window.CRM && window.CRM.toast) window.CRM.toast(failed.length ? "Restore finished with " + failed.length + " problem" + (failed.length === 1 ? "" : "s") : "Backup restored");
      restoreBtn.disabled = true;
      cancelBtn.textContent = "Done";
      if (!failed.length && window.CRM && window.CRM.rerender) setTimeout(() => window.CRM.rerender(), 1200);
    });
  ...
  async function renderZone(store) {
    ...
    zone.innerHTML = `
      ...
      <p class="hint bkp-lead">Back up every module as a downloadable file (with this device's write keys) and as a published backup on the server. Restoring validates the backup first, shows exactly what will change, and only then replaces today's data — the replaced version is archived on this device.</p>
      <div class="bkp-grid">
        <div class="bkp-col">
          <h3>Back up</h3>
    ...
    async function runFull() {
      ...
      statusShow(statusEl, "ok", "Backup complete — " + n + " module document" + (n === 1 ? "" : "s") + " (" + built.summary.records + " records, " + fmtBytes(bytes) + " file). Downloaded and published" + (pub.noop ? " — nothing had changed since the last published backup, so the server copy is untouched." : "."));
      if (window.CRM && window.CRM.toast) window.CRM.toast("Full backup saved & published");
      busy(fullBtn, false);
      dlBtn.disabled = false;
    }
    async function runDownloadOnly() {
      ...
      statusShow(statusEl, "ok", "Reading every module document…");
      const built = await store.buildBackup().catch(e => ({ ok: false, code: "error", detail: (e && e.message) || String(e) }));
      if (!built.ok) {
      ...
      const bytes = downloadSnapshot(built.snapshot);
      statusShow(statusEl, "ok", "Downloaded " + built.summary.present + " module document" + (built.summary.present === 1 ? "" : "s") + " (" + built.summary.records + " records, " + fmtBytes(bytes) + "). Nothing was written to the server.");
      if (window.CRM && window.CRM.toast) window.CRM.toast("Backup file downloaded");
      busy(dlBtn, false);
      fullBtn.disabled = false;
    }
    ...
    fileInput.addEventListener("change", async () => {
      ...
      if (!file) return;
      fileInput.value = "";
      statusShow(statusEl, "ok", "Reading " + esc(file.name) + "…");
      let text;
      try {
