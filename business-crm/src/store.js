    ...
    async function validateBackup(raw) {
      ...
      if (typeof raw === "string") {
        try {
          ...
          return { ok: false, code: "invalid_json", detail: "That file is not valid JSON, so it is not a Business CRM backup." };
        }
      }
      if (!snap || typeof snap !== "object" || Array.isArray(snap)) return { ok: false, code: "not_a_backup", detail: "That file does not contain a Business CRM backup." };
      if (snap.k !== "bcrmbak" || snap.v !== 1 || (snap.kind !== "full" && snap.kind !== "published")) {
        return { ok: false, code: "not_a_backup", detail: "That file does not look like a Business CRM backup (missing the backup marker)." };
      }
