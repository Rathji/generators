window.CRM_RECORDS = (function () {
  ...
  const PREFIX = { companies: "c", contacts: "ct", leads: "l", deals: "d", activities: "a" };
  ...
  async function findRefs(store, module, refKey, id) {
    ...
    for (const m of (store && store.modules) || []) {
      ...
      for (const r of recordsOf(doc.content)) {
        if (r && typeof r === "object" && r[refKey] !== undefined && String(r[refKey]) === s) {
          out.push({ module: m, id: r.id, name: recordName(r) });
        }
      }
    }
    return out;
