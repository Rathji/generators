...
window.CRM_REPORTS = (function () {
  ...
  async function loadAll(store) {
    const docs = {};
    const mods = ["companies", "contacts", "leads", "deals", "activities", "emails"];
    for (const m of mods) {
      try {
    ...
    const maps = {};
    for (const m of ["companies", "contacts", "deals", "leads"]) maps[m] = new Map(rec(m).map(x => [x.id, x]));
    return {
      docs,
  ...
  function printReport(rpt) {
    ...
    printSheet("Business CRM — " + rpt.title, "Generated " + rpt.generatedAtLabel + " · Report", body);
  }
  ...
  async function renderLibrary(ctx) {
    ...
    defs.forEach(def => {
      ...
      const icon = el("span", "mc-icon", meta ? meta.group === "Pipeline" ? "◈" : "◔" : "▤");
    ...
    TEMPLATES.forEach(meta => {
      ...
      const top = el("div", "rep-card-top");
      top.appendChild(el("span", "mc-icon", meta.group === "Pipeline" ? "◈" : "◔"));
      const nm = el("div", "rep-card-name");
  ...
  async function renderExport(ctx) {
    ...
    const seg = el("div", "seg");
    const modBtns = {};
    [["companies", "Companies"], ["contacts", "Contacts"], ["leads", "Leads"], ["deals", "Deals"], ["activities", "Activities"]].forEach(pair => {
      const b = document.createElement("button");
      b.type = "button";
      b.textContent = pair[1];
    ...
    async function paint() {
      ...
      allPrint.addEventListener("click", () => {
        ...
        const html = "<table class=\"rep-table\"><thead><tr>" + t.head.map(h => "<th>" + R.esc(h) + "</th>").join("") + "</tr></thead><tbody>" +
          ...
          "</tbody></table>";
        printSheet("Business CRM — " + R.moduleLabel(module) + " list", records.length + " records", html);
      });
      ...
      function paintRows(q) {
        ...
        shown.slice(0, 60).forEach(rec => {
          ...
          sheetBtn.addEventListener("click", async () => {
            ...
            printSheet("Business CRM — " + R.moduleLabel(module) + " sheet", R.recordName(rec) || rec.id, html + tl);
          });
