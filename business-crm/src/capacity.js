window.CRM_CAPACITY = (function () {
  ...
  function isDateVal(v, key) {
    ...
    const kDate = !!key && /(date|time|when|at$|^on$|_on$|closed|close|created|updated|due|won|start|end|since|until|sent|scheduled|completed|opened|deadline|year|month|day|last|next|expiry|expires|anniversary|birth|paid|signed|shipped|received|invoice|follow)/i.test(String(key));
  ...
  function recordTitle(r) {
    ...
    const keys = ["name", "title", "subject", "full_name", "company", "companyName", "email", "label", "account", "deal", "id"];
    for (const k of keys) {
      const v = r[k];
  ...
  function statusShow(el, kind, text) {
    el.innerHTML = "";
    el.hidden = false;
    el.appendChild(el2("div", "bkp-msg " + kind, text));
  }
  ...
  function recordRows(records) {
    ...
    for (const r of records) {
      ...
      det.appendChild(sum);
      const pre = el("pre", "arc-json", JSON.stringify(r, null, 1));
      det.appendChild(pre);
  ...
  async function renderZone(store) {
    ...
    zone.innerHTML = `
      ...
      <p class="hint cap-lead">Every module document lives in server files (one head + part files when it grows) under a 5&nbsp;MB ceiling per file. Old-period records can be moved out of a module into its read-only archive, freeing room while staying retrievable on demand.</p>
      ...
      <div class="cap-grid">
        <div class="cap-col">
          ...
          <div class="cap-form">
            <div class="cap-field">
              <label class="cap-label" for="">Module</label>
            ...
            <div class="cap-field">
              ...
              <input class="cap-input cap-custom" data-cap-custom placeholder="…or type a field path (e.g. closedAt, meta.won_at)" autocomplete="off">
            </div>
            <div class="cap-field">
    ...
    async function refreshFieldOptions() {
      ...
      customInput.value = "";
      if (!m) {
        fieldSel.appendChild(new Option("no module documents yet", ""));
        updateCutoffText();
        return;
      }
      let content = null;
      try {
        ...
        if (ld && (ld.state === "cache" || ld.state === "canonical" || ld.state === "dirty" || ld.state === "none") && ld.content) content = ld.content;
      } catch (e) {}
      const fields = detectDateFields(content);
      if (fields.length) {
        for (const f of fields) {
          fieldSel.appendChild(new Option(f.path + " (" + f.count + " rec)", f.path));
        }
    ...
    function moduleOptions() {
      ...
      for (const m of store.modules) {
        frag.appendChild(new Option(moduleLabel(m), m));
      }
      modSel.appendChild(frag);
    }

    async function refreshCapacity() {
      const info = await store.capacityInfo().catch(e => null);
      if (!info) {
        tbody.innerHTML = "";
      ...
      for (const m of store.modules) {
        ...
        if (row.present) {
          st.textContent = "rev " + row.revision + (row.records ? " · " + row.records + " records" : " · no records");
        } else if (row.archive) {
          st.textContent = row.localOnly ? "live doc removed here — cache only" : "no live document";
        } else {
          st.textContent = row.localOnly ? "local draft only (not on the server)" : "no document yet";
        }
        ...
        if (row.present) {
          const byt = el("span", "cap-cell", fmtBytes(row.liveBytes));
          liveTd.appendChild(byt);
          const parts = row.parts > 1 ? " · " + row.parts + " files" : " · head file";
          liveTd.appendChild(el("span", "cap-sub", parts));
        ...
        if (row.archive && row.archive.batches) {
          ...
          const bits = [];
          bits.push(fmtBytes(row.archive.bytes) + " stored");
    ...
    async function refreshArchives() {
      ...
      for (const b of sorted) {
        ...
        const title = el("span", "arc-batch-title", "Batch b" + b.no + " · " + b.count + " record" + (b.count === 1 ? "" : "s") + " archived " + fmtStamp(b.archivedAt));
        ...
        restBtn.addEventListener("click", async () => {
          ...
          if (window.CRM && window.CRM.toast) window.CRM.toast("Batch b" + b.no + " restored to live data");
          refreshCapacity();
          refreshArchives();
          refreshFieldOptions();
        });
    ...
    async function doArchive() {
      ...
      if (window.CRM && window.CRM.toast) window.CRM.toast("Archived " + res.archived + " records to batch b" + b.no);
      previewArea.innerHTML = "";
      archiveBtn.hidden = true;
      lastPreview = null;
      busy(archiveBtn, false);
    ...
    refreshCapacity().then(() => {
      ...
      chipEl.textContent = "unavailable";
      console.error("capacity zone init failed:", e);
    });
