...
// How many superseded versions of a document we keep a descriptor for. The
// chunk files themselves are never destroyed (content-addressed orphans), so
// this descriptor list is all that's needed to read an old version back and
// therefore to give every document a real version history with restore.
const HISTORY_MAX = 25;

// A compact descriptor of one document version (enough to locate + verify its
// chunks). Stored on the document's own registry entry (`meta.history`), so it
// rides along with the document through the existing session-merge/concurrency
// logic instead of needing a second registry structure.
function versionDescriptor(meta) {
  return {
    version: meta.version,
    hash: meta.hash,
    chunks: meta.chunks ? meta.chunks.slice() : [],
    bytes: meta.bytes,
...
export function createDocumentStore({
  ...
  async function _writeOne(id, data, updatedBy, updatedAt) {
    ...
    // Version history: prepend the version we're superseding (with the chunk
    // names that locate its content) so a prior version can be read back and
    // restored. Capped so the registry stays small.
    const priorHistory = prev && Array.isArray(prev.history) ? prev.history.slice() : [];
    if (prev) priorHistory.unshift(versionDescriptor(prev));
    if (priorHistory.length > HISTORY_MAX) priorHistory.length = HISTORY_MAX;
    meta.history = priorHistory;
  ...
  // ---- version history (reads back the chunk files of a superseded version) ----
  async function _history(id) {
    await loadRegistry();
    const cur = reg && reg.docs ? reg.docs[id] : null;
    if (!cur) return [];
    const prior = Array.isArray(cur.history) ? cur.history : [];
    return [{ ...versionDescriptor(cur), current: true }, ...prior.map((h) => ({ ...h, current: false }))];
  }

  async function _readVersionData(id, version) {
    await loadRegistry();
    const cur = reg && reg.docs ? reg.docs[id] : null;
    if (!cur) return null;
    const desc = cur.version === version ? versionDescriptor(cur) : (cur.history || []).find((h) => h.version === version) || null;
    if (!desc) return null;
    const parts = [];
    for (const name of desc.chunks || []) {
      const text = await channel.get(name);
      if (text == null) {
        throw new StoreError(CODES.MISSING_CHUNK, `Version ${version} of "${id}" is missing data part "${name}".`);
      }
      parts.push(text);
    }
    const json = parts.join("");
    if (desc.hash && hashString(json) !== desc.hash) {
      throw new StoreError(CODES.INTEGRITY, `Version ${version} of "${id}" failed its integrity check.`);
    }
    let data;
    try {
      data = JSON.parse(json);
    } catch {
      throw new StoreError(CODES.INTEGRITY, `Version ${version} of "${id}" is corrupt and could not be parsed.`);
    }
    return {
      id,
      version,
      ...
      current: cur.version === version,
    };
  }

  // Restoring an old version is an ordinary reversible WRITE: the content of
  // the chosen version becomes a NEW version, so the version restored *from*
  // stays in history and nothing is destroyed.
  async function _restoreVersion(id, version, opts = {}) {
    const old = await _readVersionData(id, version);
    if (!old) throw new StoreError(CODES.UNKNOWN_RECORD, `No version ${version} of "${id}".`);
    const changes = await _writeMany({ [id]: old.data }, { updatedBy: opts.updatedBy || "restore" });
    const c = changes[0];
    if (c && c.error) throw c.error;
    if (c && c.conflicted) {
      throw new StoreError(CODES.CONCURRENT_WRITE, `"${id}" changed elsewhere while restoring — re-check the version history and try again.`);
    }
    return {
      id,
      restoredFrom: version,
      version: c && c.meta ? c.meta.version : null,
      changed: !!(c && c.changed),
      doc: c && c.doc ? c.doc : null,
    };
  }
  ...
  return {
    ...
    history: (id) => withLock(() => _history(id)),
    readVersion: (id, version) => withLock(() => _readVersionData(id, version)),
    restoreVersion: (id, version, opts) => withLock(() => _restoreVersion(id, version, opts)),
