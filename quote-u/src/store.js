// ============================================================================
// quote-u — versioned document store (System-of-record storage)
// ----------------------------------------------------------------------------
// The quoting system of record is a set of versioned documents, one per entity
// group (see QU_ENTITIES). Each document lives as a canonical editable file on
// the upload host (a head file plus numbered part files when it is large) and
// as a fast local cache + locally cached edit key in kv. Every canonical write
// carries a monotonic revision and a SHA-256 content hash, so a device can
// never silently overwrite a newer revision, and a document survives reloads
// and can be reconstructed on another device from the canonical copy.
//
// Adapted from CRM-U's proven store layer (same envelope, revision guard,
// edit-key cache, part splitting, conflict detection) with quote-u's namespace.
// Sync/conflict/backup/capacity UI is layered on top in roadmap Phase 1 task 8.
// ============================================================================
const QU_ENTITIES = [
  "quotes",
  "quote_versions",
...
(function () {
  ...
  const DEFAULT_MODULES = QU_ENTITIES.slice();
  ...
  function createStore(opts) {
    ...
    const token = opts.token || tokenFrom(typeof window !== "undefined" ? window.generatorPublicId || window.generatorName || "qu" : "qu");
    ...
    function fileName(module) {
      return ["qu", slug(module), ns ? "x" + slug(ns) : null, slug(token)].filter(Boolean).join("-");
    }
    ...
    function bkpHeadName(module) {
      return ["qu", "bkp", slug(module), ns ? "x" + slug(ns) : null, slug(token)].filter(Boolean).join("-");
    }
    function bkpPartName(module, k) {
      return bkpHeadName(module) + "-p" + k;
    }
    function bkpIndexName() {
      return ["qu", "bkp", "index", ns ? "x" + slug(ns) : null, slug(token)].filter(Boolean).join("-");
    }
    function archiveIndexName(module) {
      return ["qu", "arc", slug(module), ns ? "x" + slug(ns) : null, slug(token)].filter(Boolean).join("-");
    }
    ...
    function parseHead(text, module, expectK) {
      ...
      const markerLabel = expectK === "qudoc" ? "qudoc" : "archive";
    ...
    function parsePart(text, module, partNum) {
      ...
      if (!p || p.k !== "qupart" || p.module !== module || p.part !== partNum) {
    ...
    async function getMeta() {
      ...
      if (!m || !m.deviceId) {
        m = { store: "qu", schema: 1, deviceId: randHex(10), createdAt: new Date().toISOString() };
    ...
    /* ── write guards ───────────────────────────────────────────────────────
       A module can register a synchronous guard that vetoes a canonical write
       before it happens. `guard(module, content, previous)` returns null to
       allow, or a refusal `{ ok:false, code, detail }`. Guards run on every
       guarded write path (saveChecked, the reconcile publish, conflict
       resolution and history restore), so an invariant that must hold no matter
       which module wrote a document (e.g. a frozen version is immutable) is
       enforced at the storage layer, not only in the writer. A guard that throws
       fails closed (code `guard_error`). */
    const writeGuards = [];
    function registerGuard(fn) {
      if (typeof fn !== "function") return function () {};
      writeGuards.push(fn);
      return function () {
        const i = writeGuards.indexOf(fn);
        if (i !== -1) writeGuards.splice(i, 1);
      };
    }
    function runWriteGuards(module, content, previous) {
      for (let i = 0; i < writeGuards.length; i++) {
        let res;
        try {
          res = writeGuards[i](module, content, previous === undefined ? null : previous);
        } catch (e) {
          return { ok: false, code: "guard_error", detail: (e && e.message) || String(e) };
        }
    ...
    async function writeCanonical(module, split, info) {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        const body = JSON.stringify({ k: "qupart", v: 1, module, ns, part: i + 2, content: parts[i] });
      ...
      const base = { k: "qudoc", v: 1, module, ns, revision: info.revision, updatedAt: info.updatedAt, updatedBy: info.updatedBy, parts: parts.length + 1, sha };
    ...
    async function tryPublish(module, content, info) {
      if (info && info.previous !== undefined) {
        const refused = runWriteGuards(module, content, info.previous);
    ...
    async function saveDoc(module, content) {
      try {
        ...
        // Route the low-level write through the same guards saveChecked and the
        // reconcile publish use, so a direct saveDoc cannot slip a forbidden
        // change past the storage boundary. The cached copy is the best available
        // "previous" here (a stale remote would already have been refused above).
        const guardPrevious = remotePresent && cache && cache.content ? cache.content : null;
        const guardRefusal = runWriteGuards(module, content, guardPrevious);
    ...
    async function saveChecked(module, content, opts) {
      ...
      try {
        ...
        const guardRefusal = runWriteGuards(module, content, remotePresent ? remote.content : null);
        if (guardRefusal) return guardRefusal;
        ...
        const st = await setLedger(module, { content, baseRevision: expectedBase });
        const res = await tryPublish(module, content, { revision: expectedBase + 1, expectedRemoteRev: remotePresent ? remoteRev : 0, remotePresent, previous: remotePresent ? remote.content : null });
        if (res.ok) {
    ...
    async function reconcileDoc(module) {
      ...
      if (!remote.ok) {
        if (remote.state === "none") {
          const res = await tryPublish(module, ledger.content, { revision: 1, expectedRemoteRev: 0, remotePresent: false, previous: null });
          if (res.ok) {
            await clearLedger(module);
      ...
      if (remote.revision === ledger.baseRevision) {
        ...
        const res = await tryPublish(module, ledger.content, { revision: remote.revision + 1, expectedRemoteRev: remote.revision, remotePresent: true, previous: remote.content });
        if (res.ok) {
          await clearLedger(module);
    ...
    async function pushHistory(module, rec, ev) {
      ...
      await kvSet(kHistory(module), trimHistory(h));
    }

    /* Publish a version plucked out of the resolution history as the live
       document, at a fresh revision, and archive the version it replaced.
       Used by the version-history panel (roadmap task 8). */
    async function restoreHistoryVersion(module, opts) {
      opts = opts || {};
      try {
        const content = opts.content;
        if (!content || typeof content !== "object" || Array.isArray(content)) {
          return { ok: false, code: "bad_content", detail: "the version to restore must be a JSON object" };
        }
        const conflict = await kvGet(kConflict(module));
        const ledger = await kvGet(kLedger(module));
        if (conflict || (ledger && ledger.content)) {
          return { ok: false, code: "pending_changes", module, detail: "Finish the pending sync item for " + module + " before restoring an older version, so nothing unsaved is discarded." };
        }
        const remote = await readDoc(module);
        if (!remote.ok && remote.state !== "none") return remote;
        const headKey = await kvGet(kKey(fileName(module)));
        if (remote.ok && !headKey) {
          return { ok: false, code: "no_edit_key", detail: `This device has no write key for the ${module} document. Restore from a backup to claim ownership before restoring an older version.` };
        }
        const remotePresent = remote.ok;
        const remoteRev = remote.ok ? remote.revision : 0;
        const res = await tryPublish(module, content, { revision: remoteRev + 1, expectedRemoteRev: remoteRev, remotePresent, previous: remotePresent ? remote.content : null });
        if (!res.ok) return res;
        await cacheDoc(module, { content, revision: res.revision, updatedAt: res.updatedAt, updatedBy: res.updatedBy, parts: res.parts, lastText: res.canonicalText });
        let h = await kvGet(kHistory(module));
        h = Array.isArray(h) ? h : [];
        h.unshift({
          ...
          previous: remotePresent ? { revision: remoteRev, content: remote.content } : null,
          restoredRevision: res.revision,
          at: new Date().toISOString()
        });
        await kvSet(kHistory(module), trimHistory(h));
        lastSyncResult = lastSyncResult || {};
        lastSyncResult[module] = { state: "synced", module, revision: res.revision };
        return { ok: true, revision: res.revision, previousRevision: remotePresent ? remoteRev : null, parts: res.parts };
      } catch (e) {
    ...
    async function resolveConflict(module, choice, opts) {
      ...
      try {
        ...
        const res = await tryPublish(module, mineContent, { revision: remoteRev + 1, expectedRemoteRev: remoteRev, remotePresent: true, previous: remote.content });
        if (!res.ok) return res;
    ...
    function parseIndexText(text) {
      ...
      if (!meta || meta.k !== "qubkp" || meta.v !== 1 || !Array.isArray(meta.modules)) {
    ...
    async function validateBackup(raw) {
      ...
      if (typeof raw === "string") {
        try {
          ...
          return { ok: false, code: "invalid_json", detail: "That file is not valid JSON, so it is not a quote-u backup." };
        }
      }
      if (!snap || typeof snap !== "object" || Array.isArray(snap)) return { ok: false, code: "not_a_backup", detail: "That file does not contain a quote-u backup." };
      if (snap.k !== "qubak" || snap.v !== 1 || (snap.kind !== "full" && snap.kind !== "published")) {
        return { ok: false, code: "not_a_backup", detail: "That file does not look like a quote-u backup (missing the backup marker)." };
      }
    ...
    async function backupFingerprint(snap) {
      ...
      return digestHex("qubkp:" + ns + ":" + parts);
    }
    ...
    async function latestPublishedBackup() {
      ...
      if (raw.error) return raw.error;
      if (raw.text === null) return { ok: false, state: "none", code: "no_published_backup", detail: "No backup has been published for this quote-u workspace yet." };
    ...
    async function readPublishedBackup() {
      ...
      return { ok: true, snapshot: { k: "qubak", v: 1, kind: "published", id: meta.id, at: meta.at, ns, deviceId: meta.deviceId, generator: meta.generator, modules: mods } };
    }
    ...
    async function restoreBackup(raw) {
      ...
      for (const m of modules) {
        ...
        const restoreRefusal = runWriteGuards(m, me.content, prevContent);
        if (restoreRefusal) {
          row.action = "failed";
          row.code = restoreRefusal.code;
          row.detail = (restoreRefusal.detail || restoreRefusal.code) + " Nothing was changed for " + m + " — the restore was refused by a write guard.";
          results[m] = row;
          continue;
        }
    ...
    async function readArchiveIndex(module) {
      ...
      if (!idx || idx.k !== "quarc" || idx.v !== 1 || idx.module !== module || !Array.isArray(idx.batches) || typeof idx.nextNo !== "number") {
    ...
    async function readArchiveBatchFile(module, no) {
      return readFileset(module, () => archiveBatchName(module, no), (_m, k) => archiveBatchPartName(module, no, k), "archived batch", "quarcb");
    }
    ...
    async function archiveRecords(module, opts) {
      ...
      try {
        ...
        if (!idx) {
          idx = { k: "quarc", v: 1, module, ns, readOnly: true, nextNo: 1, updatedAt: new Date().toISOString(), updatedBy: by, batches: [] };
        }
  ...
  function createDefault(opts) {
    ...
    const kvReal = {
      get: key => r.kv.qu.get(key),
      set: (key, val) => r.kv.qu.set(key, val),
      delete: key => r.kv.qu.delete(key)
    };
    const edReal = {
    ...
    return createStore(
      Object.assign({}, opts, {
        ...
        token: tokenFrom(window.generatorPublicId || window.generatorName || "qu")
      })
    );
  }

  window.QU_STORE = { create: createStore, createDefault, tokenFrom, randHex, digestHex, DEFAULT_MODULES: DEFAULT_MODULES.slice(), EDITABLE_MAX };
})();
