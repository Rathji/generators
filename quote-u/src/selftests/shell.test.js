(function () {
  ...
  const EXPECTED = ["quotes", "builder", "catalog", "portal", "approvals", "invoicing", "connectors", "reports", "admin"];
  ...
  T.register("side navigation lists every quote-u station", () => {
  ...
  T.register("side navigation labels match station names", () => {
    const bad = [];
    for (const m of window.QU_MODULES) {
  ...
  T.register("default landing is the Quotes station", async () => {
    window.QU.go("quotes");
    await window.QU.ready();
    const el = document.getElementById("viewRoot");
    const ok = el.dataset.state === "ready" && el.querySelector(".page-title") && el.querySelector(".page-title").textContent === "Quotes";
    return ok ? true : { pass: false, detail: "state=" + el.dataset.state };
  });

  T.register("every station navigates without error", async () => {
    const fails = [];
    for (const id of EXPECTED) {
      window.QU.go(id);
      await window.QU.ready();
      const el = document.getElementById("viewRoot");
      const label = window.QU_MODULES.find(m => m.id === id).label;
  ...
  T.register("loading state shows while a station renders", async () => {
    const view = document.getElementById("viewRoot");
    const p = window.QU.go("catalog");
  ...
  T.register("a superseded station render does not settle ready() early", async () => {
    let startedResolve;
    const started = new Promise(r => { startedResolve = r; });
    let release;
    const gate = new Promise(r => { release = r; });
    const id = "slowtest";
    window.QU.addModule({ id, label: "Slow test", icon: "", render: async () => {
      startedResolve();
      await gate;
      const d = document.createElement("div");
      d.className = "page-title";
      d.textContent = "Slow test";
      return d;
    } });
    window.QU.go(id);
    await started;
    const superseding = window.QU.go("quotes");
    let settledEarly = false;
    const waiter = window.QU.ready().then(() => { settledEarly = true; });
    release();
    await new Promise(r => setTimeout(r, 40));
    const early = settledEarly;
    await superseding;
    const state = document.getElementById("viewRoot").dataset.state;
    window.QU.removeModule(id);
    await window.QU.go("quotes");
    await waiter.catch(() => {});
    return !early && state === "ready" ? true : { pass: false, detail: `settledEarly=${early} finalState=${state}` };
  });

  T.register("placeholder stations render the empty state", async () => {
    const id = "emptytest";
    window.QU.addModule({ id, label: "Empty test", icon: "", empty: { title: "Nothing here yet", message: "This temporary station has no renderer, so the shell shows its empty state." } });
    window.QU.go(id);
    await window.QU.ready();
    const el = document.getElementById("viewRoot");
  ...
  T.register("a failing station render shows the error state", async () => {
    window.QU.addModule({ id: "boomtest", label: "Boom test", icon: "", render: () => { throw new Error("deliberate test failure"); } });
    window.QU.go("boomtest");
    await window.QU.ready();
    const el = document.getElementById("viewRoot");
    ...
    window.QU.removeModule("boomtest");
    window.QU.go("quotes");
    await window.QU.ready();
  ...
  T.register("unknown routes show an actionable error", async () => {
    window.QU.go("nonexistent-station");
    await window.QU.ready();
    const el = document.getElementById("viewRoot");
    const errCard = el.querySelector(".view-error");
    const mentions = errCard && errCard.textContent.includes("nonexistent-station");
    window.QU.go("quotes");
    await window.QU.ready();
  ...
  T.register("active nav item tracks the current station", async () => {
    window.QU.go("approvals");
    await window.QU.ready();
    const active = document.querySelectorAll('#sideNav .side-link[aria-current="page"]');
    const ok = active.length === 1 && active[0].dataset.id === "approvals";
    window.QU.go("quotes");
    await window.QU.ready();
    return ok ? true : { pass: false, detail: "expected exactly one active link on approvals" };
  });

  T.register("per-station pages have consistent page-head structure", async () => {
    const fails = [];
    for (const id of ["quotes", "builder", "reports"]) {
      window.QU.go(id);
      await window.QU.ready();
      const el = document.getElementById("viewRoot");
  ...
  T.register("document title is namespaced to quote-u", async () => {
    window.QU.go("admin");
    await window.QU.ready();
    const ok = document.title === "Admin · quote-u";
    window.QU.go("quotes");
    await window.QU.ready();
    return ok ? true : { pass: false, detail: "title=" + document.title };
  });
})();
