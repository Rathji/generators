// quote-u — multi-device sync & conflict resolution tests (roadmap task 8)
(function () {
  ...
  function mockTransport(opts) {
    ...
    const ns = opts.ns || "sd" + QS.randHex(6);
    const mods = ["quotes"];
    const base = { ns, editable, modules: mods };
    const storeA = QS.create(Object.assign({}, base, { kv: kvOf(mapsA) }, opts));
    const storeB = QS.create(Object.assign({}, base, { kv: kvOf(mapsB) }));
  ...
  async function makeConflict(p, mineContent) {
    ...
    const r1 = await A.saveChecked("quotes", c1, { expectedBase: 0 });
    assert(r1.ok && r1.revision === 1 && r1.created, "A create failed: " + JSON.stringify(r1));
    p.exportKeysToB();
    const bl = await B.loadDoc("quotes");
    ...
    const r2 = await A.saveChecked("quotes", c2, { expectedBase: 1 });
    ...
    const head = headOf(p.files, A, "quotes");
  ...
  T.register("sync: saveChecked requires the loaded revision (expectedBase)", async () => {
    ...
    const r = await p.storeA.saveChecked("quotes", { records: [] }, {});
  ...
  T.register("sync: two-device divergence stages mine, keeps canonical, reports conflict", async () => {
    ...
    const B = p.storeB;
    const dirty = await B.loadDoc("quotes");
  ...
  T.register("sync: keep-mine publishes the local version and archives theirs", async () => {
    ...
    const rr = await p.storeB.resolveConflict("quotes", "keepMine");
    if (!rr.ok) return { pass: false, detail: JSON.stringify(rr) };
    const head = headOf(p.files, p.storeA, "quotes");
    ...
    const hist = await p.storeB.listHistory("quotes");
    ...
    const ld = await p.storeB.loadDoc("quotes");
  ...
  T.register("sync: keep-theirs keeps canonical and archives mine", async () => {
    const p = mockTransport();
    const { mine, theirs } = await makeConflict(p);
    const rr = await p.storeB.resolveConflict("quotes", "keepTheirs");
  ...
  T.register("sync: startup reconciliation flags a cross-device conflict and writes nothing", async () => {
    ...
    await B.stageEdit("quotes", mine);
    const c2 = { records: [{ id: 1, stage: "won" }, { id: 2 }] };
    await A.saveChecked("quotes", c2, { expectedBase: 1 });
    const B2 = QS.create({ ns: p.ns, kv: p.kvB, editable: p.editable, modules: ["quotes"] });
  ...
  T.register("sync: field-level merge combines both sides with per-field picks", async () => {
    ...
    const md = await B.mergeDiff("quotes");
    ...
    const built = await B.buildMerged("quotes", picks);
    ...
    const rr = await B.resolveConflict("quotes", "merge", { merged: m, picks });
  ...
  T.register("sync: a conflict that moved while under review is never overwritten", async () => {
    ...
    const { mine } = await makeConflict(p);
    const A = p.storeA;
    await A.saveChecked("quotes", { records: [{ id: 9, stage: "third-writer" }] }, { expectedBase: 2 });
    const rr = await p.storeB.resolveConflict("quotes", "keepMine");
    ...
    const dirty = await p.storeB.loadDoc("quotes");
  ...
  T.register("sync: resolution UI renders a conflict and keep-mine resolves it", async () => {
    const p = mockTransport();
    const { mine } = await makeConflict(p);
    if (!window.QU_SYNC) return { pass: true, skip: true, detail: "syncpanel not loaded" };
    const zone = await window.QU_SYNC.conflictBlocks(p.storeB, { onChange: null });
    ...
    if (zone.textContent.indexOf("Quotes") === -1) return { pass: false, detail: "module label missing" };
  ...
  T.register("sync: field-level merge flow works through the UI", async () => {
    ...
    if (!window.QU_SYNC) return { pass: true, skip: true, detail: "syncpanel not loaded" };
    const zone = await window.QU_SYNC.conflictBlocks(B, { onChange: null });
  ...
  T.register("sync: statusSummary tracks pending and synced states", async () => {
    ...
    p.exportKeysToB();
    await B.loadDoc("quotes");
    await B.stageEdit("quotes", { records: [{ id: 1 }, { id: 2 }] });
    const sum = await B.statusSummary();
  ...
  T.register("sync: real two-device conflict + keep-theirs (needs saved generator)", async () => {
    ...
    const ns = "qyl" + Date.now().toString(36) + QS.randHex(4);
    const genToken = QS.tokenFrom(window.generatorPublicId || window.generatorName || "qu");
    ...
    let A, B;
    try {
      A = QS.create({ ns, kv: { get: k => r.kv.qu.get(k), set: (k, v) => r.kv.qu.set(k, v), delete: k => r.kv.qu.delete(k) }, editable: edReal, modules: ["livesync"], generatorName: genName, token: genToken });
      B = QS.create({ ns, kv: { get: k => r.kv.qub.get(k), set: (k, v) => r.kv.qub.set(k, v), delete: k => r.kv.qub.delete(k) }, editable: edReal, modules: ["livesync"], generatorName: genName, token: genToken });
    } catch (e) {
    ...
    if (r1.code === "over_daily_allowance") return { pass: true, skip: true, detail: "upload daily allowance exhausted — live sync check skipped" };
    ...
    const keysA = await r.kv.qu.entries().catch(() => []);
    for (const pair of keysA) {
      const k = pair[0];
      if (k.startsWith("editkey:" + ns + ":")) await r.kv.qub.set(k, pair[1]);
    }
    ...
    const cleanup = async () => {
      for (const folder of [r.kv.qu, r.kv.qub]) {
