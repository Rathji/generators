(function () {
  ...
  const ID = window.QU_INVOICEDIRECT;
  ...
  function makeEnv(opts) {
    ...
    const invoiceDirect = ID.createService({ outbox, gateway, invoiceIntents, quotes, versions, audit, policy: opts.invoicePolicy || null, scope: "*" });
    ...
    return { ns, store, audit, gateway, quotes, prices, versions, portalTokens, send, recompute, events, approvals, outbox, oppSync, oppNote, revenue, invoiceIntents, invoiceDirect, actions };
  }
  ...
  T.register("direct invoice: the approved selection becomes pre-tax invoice lines and an accounting-customer payload", () => {
    const bad = [];
    const lines = [
    ...
    const built = ID.buildInvoiceLines({ line_items: lines, selection: ["l1", "l2"], currency: "CAD" });
    if (built.length !== 2) bad.push("lines should be the accepted lines only: " + built.length);
    const r1 = built.find(r => r.description === "Router");
    if (!r1 || r1.amount_cents !== 30000 || r1.unit_price_cents !== 15000 || r1.recurring !== false) bad.push("Router line: " + JSON.stringify(r1));
    const r2 = built.find(r => r.description === "Managed IT");
    if (!r2 || r2.amount_cents !== 20000 || r2.recurring !== true || r2.kind !== "mrr") bad.push("Managed IT line: " + JSON.stringify(r2));

    const customer = ID.buildCustomerPayload({ quote: { company_id: "c1" }, company_id: "c1" });
    if (customer.external_key !== "c1" || customer.company_id !== "c1") bad.push("customer payload: " + JSON.stringify(customer));
    const mapped = ID.buildCustomerPayload({ quote: { company_id: "c1", accounting_customer_key: "acct-xyz" } });
    if (mapped.external_key !== "acct-xyz") bad.push("an explicit mapping should win: " + JSON.stringify(mapped));

    const payload = ID.buildPayload({ version: { id: "v1" }, quote: { id: "q1", company_id: "c1" }, line_items: lines, selection: ["l1", "l2"], totals: { currency: "CAD" } });
    if (!payload.ok) bad.push("valid payload refused: " + JSON.stringify(payload));
    else {
      if (payload.payload.idempotency_key !== "v1:invoice_direct") bad.push("key: " + payload.payload.idempotency_key);
      ...
      if (payload.payload.path !== "direct") bad.push("path: " + payload.payload.path);
      if (payload.payload.customer.external_key !== "c1") bad.push("customer key missing");
    }
    const noVersion = ID.buildPayload({ quote: { id: "q1", company_id: "c1" }, line_items: lines, selection: ["l1"] });
    if (noVersion.ok || noVersion.code !== "version_required") bad.push("missing version not refused: " + JSON.stringify(noVersion));
    const noCustomer = ID.buildPayload({ version: { id: "v1" }, quote: { id: "q1" }, line_items: lines, selection: ["l1"] });
    if (noCustomer.ok || noCustomer.code !== "no_customer") bad.push("missing company not refused: " + JSON.stringify(noCustomer));
    const noLines = ID.buildPayload({ version: { id: "v1" }, quote: { id: "q1", company_id: "c1" }, line_items: [lines[2]], selection: [] });
    ...
    return bad.length ? { pass: false, detail: bad.join(" | ") } : { pass: true, detail: "the accepted lines become pre-tax invoice lines, the customer key resolves from the company mapping, and a missing version/company/lines is refused" };
  });

  T.register("direct invoice: approval → customer upsert → invoice → intent reconciled, audited once, and idempotent", async () => {
    const env = makeEnv();
    ...
    if (!run.ok || run.succeeded !== 1) bad.push("job did not run: " + JSON.stringify(run));

    const inv = await env.gateway.call("accounting", "getInvoice", { version_id: s.version.id }, { scope: "*" });
    if (!inv.ok || !inv.result) bad.push("no invoice created: " + JSON.stringify(inv));
    else {
      ...
      if (inv.result.customer_id !== "acct-c1") bad.push("invoice customer: " + inv.result.customer_id);
    }
    const cust = await env.gateway.call("accounting", "getCustomer", { external_key: "c1" }, { scope: "*" });
    if (!cust.ok || !cust.result || cust.result.id !== "acct-c1") bad.push("accounting customer not upserted: " + JSON.stringify(cust));
    ...
    if (!int.ok || !int.intent) bad.push("no intent row: " + JSON.stringify(int));
      ...
      if (int.intent.path !== "direct") bad.push("intent path: " + int.intent.path);
      if (int.intent.status !== "created") bad.push("intent status: " + int.intent.status);
      if (int.intent.external_system !== "accounting") bad.push("intent external_system: " + int.intent.external_system);
    ...
    else if (evs[0].actor_type !== "system" || evs[0].detail.path !== "direct") bad.push("invoice_created audit shape: " + JSON.stringify(evs[0].detail));

    // Idempotency: a re-enqueue dedupes and a re-run cannot create a second invoice.
    const again = await env.invoiceDirect.enqueueForVersion({ quote: approved.quote, version: s.version });
    ...
    const cnt = await env.gateway.call("accounting", "invoiceCount", {}, { scope: "*" });
    if (cnt.result !== 1) bad.push("a duplicate invoice was created: " + cnt.result);
    // The handler on an already-created intent is a clean skip.
    ...
    return bad.length ? { pass: false, detail: bad.join(" | ") } : { pass: true, detail: "approval → direct enqueue → customer upsert + invoice create → the intent reconciles the accounting reference and one invoice_created is audited; re-running creates no second invoice" };
  });

  T.register("direct invoice: the intent guard refuses a version already claimed on the other path, and a disabled flag skips the path", async () => {
    const env = makeEnv();
    const bad = [];
    // The PSA path claims the version first (as if it won the race).
    const claimed = await env.invoiceIntents.claim({ quote_id: "qr", version_id: "vr", path: "psa", amount_cents: 50000 });
    if (!claimed.ok) return { pass: false, detail: "seed claim failed: " + JSON.stringify(claimed) };
    const built = ID.buildPayload({ version: { id: "vr" }, quote: { id: "qr", company_id: "c1" }, line_items: [{ id: "l1", description: "Router", quantity: 1, unit_sell_cents: 50000 }], selection: ["l1"] });
    ...
    if (!entry || entry.ok) bad.push("the direct job should have failed: " + JSON.stringify(entry));
    ...
    const cnt = await env.gateway.call("accounting", "invoiceCount", {}, { scope: "*" });
    if (cnt.result !== 0) bad.push("an invoice was created despite the guard: " + cnt.result);
    const gi = await env.invoiceIntents.getForVersion("vr");
    if (!gi.ok || !gi.intent || gi.intent.path !== "psa" || gi.intent.status !== "pending") bad.push("the winning intent changed: " + JSON.stringify(gi.intent));

    // A disabled policy skips enqueueing entirely.
    const off = makeEnv({ invoicePolicy: { write_invoice: false } });
    const s = await seedLinked(off);
    if (!s.ok) return { pass: false, detail: "seed failed" };
    const ap = await off.actions.approve(s.secret, { selection: [], approver_name: "Dana" }, { ip: "203.0.113.9" });
    if (!ap.ok) return { pass: false, detail: "approve failed" };
    const skipped = await off.invoiceDirect.enqueueForVersion({ quote: ap.quote, version: s.version });
    if (!skipped.ok || skipped.skipped !== true || skipped.reason !== "disabled") bad.push("disabled policy did not skip: " + JSON.stringify(skipped));
    return bad.length ? { pass: false, detail: bad.join(" | ") } : { pass: true, detail: "a version already claimed on the psa path makes the direct job fail terminally with no invoice created, and a disabled write_invoice flag skips the path entirely" };
  });
})();
