...
export function button(options = {}) {
  ...
  // `icon` may be an icon name (resolved against the framework icon set) or a
  // ready-made Node — member generators often supply their own richer icon set.
  const iconNode = icon instanceof Node ? icon : icon ? svgIcon(icon, { size: size === "sm" ? 14 : 16 }) : null;
  const btn = h(
    "button",
    {
