...
import { appTitle, appSlug } from "./ui.js";
...
export async function runPhase1Tests() {
  ...
  try {
    ...
    ok(dump.app === "pm-u" && dump.schemaVersion === SCHEMA_VERSION, "export includes app marker + schema version");
...
// ── UI-kit tests: Phase 3 view integration (focus toasts, palette search,
//    branding → theme tokens) ──
export async function runUiKitTests() {
  const results = [];
  const ok = (cond, name, extra) => results.push({ ok: !!cond, name, extra });
  const folder = "pm_test_ui_" + Math.random().toString(36).slice(2, 9);
  const store = new Store({ kv: root.kv, folder, debounceMs: 5 });
  const UI = await import("./ui.js");
  const P = await import("./palette.js");
  try {
    await store.load();
    store.create("task", { title: "Unique pal task", status: "Active", priority: "low", tags: ["palette"], notes: "" });
    store.create("note", { title: "Unique pal note", body: "hello", pinned: false, tags: [] });
    store.create("project", { name: "Unique pal project", color: "#8b5cf6", status: "Active" });
    await store.save();

    // palette (task 12) — searching + command parsing, exercised without touching
    // the shared #paletteRoot overlay
    const pal = new P.Palette(store, { navigate() {}, cycleTheme() {}, openItem() {} });
    pal.input = { value: "uniquepal" };
    const hits = pal.collect();
    ok(hits.length === 3, "palette search finds records across types (" + hits.length + "/3)");
    ok(hits.every((h) => /Unique pal/.test(h.rec.title || h.rec.name)), "palette matches the query against titles/names");
    pal.input = { value: "palette" };
    ok(pal.collect().some((h) => h.type === "task"), "palette matches tag text too");
    pal.input = { value: "/new task Buy milk" };
    const cmd = pal.commandDefs();
    ok(cmd.exact && cmd.exact.id === "task" && cmd.text === "Buy milk", "palette parses '/new task <title>' commands");
    pal.input = { value: "/foc" };
    ok((pal.commandDefs().partial || []).some((c) => c.id === "focus"), "palette suggests partial commands");
    pal.input = { value: "zzzznope" };
    ok(pal.collect().length === 0, "palette returns nothing for a non-match");

    // template-u notification channel (task 11) — notify() forwards title + action
    const prevToaster = globalThis.PU && PU.app ? PU.app.toaster : null;
    let seen = null;
    UI.setToaster({ show: (msg, opts) => { seen = { msg, ...opts }; return { id: "x" }; } });
    const handle = UI.notify("Session done", { variant: "success", title: "Focus session complete", action: { label: "Start", onClick() {} } });
    ok(seen && seen.msg === "Session done" && seen.type === "success" && seen.title === "Focus session complete", "notify() forwards message, type and title to the template-u toaster");
    ok(seen.action && seen.action.label === "Start", "notify() forwards an inline action button");
    ok(handle && handle.id === "x", "notify() returns the toaster handle");
    UI.setToaster(prevToaster);

    // branding → theme tokens (task 14) — runtime overrides win over main.pjs config
    const brand = globalThis.PU.branding.createBranding({ primary: "#ff0000", appTitle: "Custom" }, { config: { appTitle: "Base", companyName: "Acme" } });
    const b = brand.get();
    ok(b.appTitle === "Custom" && b.companyName === "Acme", "branding merges runtime overrides over main.pjs config");
    ok(brand.tokens()["--pu-primary"] === "#ff0000", "branding exposes the primary colour as a theme token");
    ok(typeof brand.tokens()["--pu-primary-contrast"] === "string", "branding derives a contrasting text colour");
...
export async function runAllTests() {
  ...
  const r = await runUiKitTests();
  const lf = await runLocalFirstTests();
  const bh = await runBackupHistoryTests();
  const qc = await runQuickCaptureTests();
  const ig = await runIntegrateTests();
  const idn = await runIdentityTests();
  const mig = await runMigrationTests();
  const suites = [a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, lf, bh, qc, ig, idn, mig];
  const total = suites.reduce((x, s) => x + s.total, 0);
  const passed = suites.reduce((x, s) => x + s.passed, 0);
  return { dataLayer: a.passed + "/" + a.total, phase1: b.passed + "/" + b.total, phase2: c.passed + "/" + c.total, phase2b: d.passed + "/" + d.total, phase3: e.passed + "/" + e.total, phase3b: f.passed + "/" + f.total, phase3c: g.passed + "/" + g.total, phase4: h.passed + "/" + h.total, phase4b: i.passed + "/" + i.total, phase5: j.passed + "/" + j.total, phase6: k.passed + "/" + k.total, phase7: l.passed + "/" + l.total, phase8: m.passed + "/" + m.total, phase9: n.passed + "/" + n.total, phase10: o.passed + "/" + o.total, phase11: p.passed + "/" + p.total, phase12: q.passed + "/" + q.total, uiKit: r.passed + "/" + r.total, localFirst: lf.passed + "/" + lf.total, backupHistory: bh.passed + "/" + bh.total, quickCapture: qc.passed + "/" + qc.total, integrate: ig.passed + "/" + ig.total, identity: idn.passed + "/" + idn.total, migration: mig.passed + "/" + mig.total, total: passed + "/" + total };
}
...
export async function runPhase7Tests() {
  ...
  try {
    ...
    // dashboard strip (task 56): only opted-in habits, alphabetical
    ok(H.habitsForDashboard(store).length === 0, "habitsForDashboard is empty when nothing is opted in");
    store.upsert("habit", h2.id, { showDashboard: true });   // Jog
    store.upsert("habit", h3.id, { showDashboard: true });   // No sugar
    const dash = H.habitsForDashboard(store);
    ok(dash.length === 2, "habitsForDashboard returns only showDashboard habits");
    ok(dash.map((x) => x.name).join(",") === "Jog,No sugar", "habitsForDashboard sorts alphabetically — got " + dash.map((x) => x.name).join(","));
    store.upsert("habit", h2.id, { showDashboard: false });
    ok(H.habitsForDashboard(store).length === 1, "un-opting a habit removes it from the strip");
...
export async function runPhase9Tests() {
  ...
  try {
    ...
    // ensureBoardData — legacy/corrupt records without `data` are seeded, not fatal
    const bare = store.create("board", { name: "Legacy mindmap", kind: "mindmap" });
    ok(bare.data === undefined, "board created without data starts undefined");
    ok(B.ensureBoardData(bare) === bare && bare.data && bare.data.center === "Main idea", "ensureBoardData seeds default data for its kind");
    const kept = store.create("board", { name: "Has data", kind: "proscons", data: { pros: ["x"], cons: [], nextId: 5 } });
    B.ensureBoardData(kept);
    ok(kept.data.pros.length === 1 && kept.data.nextId === 5, "ensureBoardData leaves existing data untouched");
    ok(B.ensureBoardData(null) === null, "ensureBoardData tolerates a null board");
...
export async function runPhase12Tests() {
  ...
  try {
    ...
    ok(md.startsWith("# " + appTitle() + " — Notes export"), "notesToMD: title line");
    ...
    ok(!!uidLine && uidLine.endsWith("@" + appSlug()), "ics: UIDs are stable + namespaced");
...
// ── Phase 4 tests (Roadmap 16): local-first storage ──────────────
export async function runLocalFirstTests() {
  const results = [];
  const ok = (cond, name, extra) => results.push({ ok: !!cond, name, extra });
  const folder = "pm_test_lf_" + Math.random().toString(36).slice(2, 9);
  const store = new Store({ kv: root.kv, folder, debounceMs: 5 });
  const cleanup = async (s) => { for (const [k] of await s.kvFolder.entries()) await s.kvFolder.delete(k); };
  try {
    await store.load();

    // one record of every entity type + a setting, then persist
    const created = {};
    for (const t of ENTITY_TYPES) created[t] = store.create(t, { title: "LF " + t, name: "LF " + t });
    ok(Object.keys(created).length === ENTITY_TYPES.length, "created one record of each of the " + ENTITY_TYPES.length + " entity types");
    store.setSetting("profileName", "Local Tester");
    await store.save();

    // everything landed in the kv (IndexedDB) folder
    const entries = await store.kvFolder.entries();
    const keySet = new Set(entries.map(([k]) => k));
    let allPresent = true;
    for (const t of ENTITY_TYPES) if (!keySet.has(store.key(t, created[t].id))) allPresent = false;
    ok(allPresent, "every created record is stored in the kv folder");
    ok(keySet.has("__settings") && keySet.has("__meta"), "settings + meta persisted alongside records");
    const metaEntry = entries.find(([k]) => k === "__meta");
    ok(metaEntry && metaEntry[1] && metaEntry[1].schemaVersion === SCHEMA_VERSION && typeof metaEntry[1].savedAt === "number", "meta records schema version + save time");

    // reload: same folder, fresh instance, full round-trip
    const store2 = new Store({ kv: root.kv, folder });
    await store2.load();
    let roundTrip = true;
    for (const t of ENTITY_TYPES) {
      const r = store2.get(t, created[t].id);
      if (!r || r.title !== "LF " + t) roundTrip = false;
    }
    ok(roundTrip, "all records round-trip through kv on reload");
    ok(store2.settings.profileName === "Local Tester", "settings round-trip through kv on reload");

    // the records live in IndexedDB, NOT in localStorage
    const probeId = created.task.id;
    const probeTitle = created.task.title;
    let foundInLS = false;
    for (let i = 0; i < localStorage.length; i++) {
      const v = localStorage.getItem(localStorage.key(i)) || "";
      if (v.includes(probeId) || v.includes(probeTitle)) foundInLS = true;
    }
    ok(!foundInLS, "record ids/titles are absent from localStorage (they live in IndexedDB)");

    // exportAll + validateBackup are pure/local — no writes, no network
    const keysBefore = new Set((await store2.kvFolder.entries()).map(([k]) => k));
    const dump = store2.exportAll();
    ok(dump.app && Array.isArray(dump.entities.task) && dump.entities.task.length >= 1, "exportAll produces a full local payload");
    const store3 = new Store({ kv: root.kv, folder });
    await store3.load();
    let threwInvalid = false;
    try { store3.validateBackup({ app: "not-pm" }); } catch (e) { threwInvalid = true; }
    ok(threwInvalid, "validateBackup rejects a foreign payload");
    const keysAfter = new Set((await store2.kvFolder.entries()).map(([k]) => k));
    ok(keysBefore.size === keysAfter.size, "exportAll/validateBackup touch neither storage nor data");

    // zero-network probe: a full lifecycle must not hit fetch/XHR/sendBeacon
    const captured = [];
    ...
    window.fetch = function (input, init) {
      try { captured.push(typeof input === "string" ? input : (input && input.url) || ""); if (init && init.body) captured.push(String(init.body)); } catch (e) {}
      return origFetch.apply(this, arguments);
    };
    XMLHttpRequest.prototype.open = function (method, url) { try { this.__url = url; } catch (e) {} return origOpen.apply(this, arguments); };
    XMLHttpRequest.prototype.send = function (body) { try { captured.push(this.__url || ""); if (body) captured.push(String(body)); } catch (e) {} return origSend.apply(this, arguments); };
    if (origBeacon) navigator.sendBeacon = function (url, data) { try { captured.push(String(url)); if (data) captured.push(String(data)); } catch (e) {} return origBeacon.apply(this, arguments); };
    try {
      const reqId = "probe-" + Math.random().toString(36).slice(2, 10);
      const reqTitle = "SECRETPROBE" + Math.random().toString(36).slice(2, 10);
      const s3 = new Store({ kv: root.kv, folder, debounceMs: 5 });
      await s3.load();
      s3.create("task", { id: reqId, title: reqTitle });
      await s3.save();
      const s4 = new Store({ kv: root.kv, folder, debounceMs: 5 });
      await s4.load();
      s4.exportAll();
      s4.remove("task", reqId);
      await s4.save();
      ok(!captured.some((t) => t.includes(reqId) || t.includes(reqTitle)), "no fetch/XHR/beacon carried record data during a full lifecycle (" + captured.length + " unrelated request(s) seen)");
    } finally {
    ...
    // wipeAll clears persisted records but keeps settings
    store2.wipeAll();
    await store2.save();
    const store5 = new Store({ kv: root.kv, folder });
    await store5.load();
    const remaining = ENTITY_TYPES.reduce((n, t) => n + store5.count(t), 0);
    ok(remaining === 0, "wipeAll removes every persisted record");
    ok(store5.settings.profileName === "Local Tester", "wipeAll keeps settings");

    await cleanup(store5);
  } catch (e) {
    ok(false, "test run threw: " + e.message, e.stack);
  }
  const passed = results.filter((r) => r.ok).length;
...
// ── Phase 4 tests (Roadmap 17): backup history ──────────────────
export async function runBackupHistoryTests() {
  const results = [];
  const ok = (cond, name, extra) => results.push({ ok: !!cond, name, extra });
  const B = await import("./backup.js");
  const folder = "pm_test_bh_" + Math.random().toString(36).slice(2, 9);
  const store = new Store({ kv: root.kv, folder, debounceMs: 5 });
  const lsKey = "pm_last_snapshot_" + folder;
  const cleanup = async () => {
    const bf = store.kv["pm_backups__" + folder];
    for (const [k] of await bf.entries()) await bf.delete(k);
    for (const [k] of await store.kvFolder.entries()) await store.kvFolder.delete(k);
  };
  try {
    await store.load();
    const bf = await B.backupFolder(store);
    ok(!!bf, "backupFolder resolves for a namespaced store folder");
    ok((await B.backupFolder({ kv: root.kv, folder: "pm" })) != null, "default store resolves the plain pm_backups folder");

    store.create("task", { title: "Alpha" });
    await store.save();
    const snap1 = await B.takeSnapshot(store);
    ok(snap1 && /^snap-/.test(snap1.key) && snap1.count === 1, "takeSnapshot writes a snap- key and counts records");
    const rawKeys = (await store.kv["pm_backups__" + folder].entries()).map(([k]) => k);
    ok(rawKeys.includes(snap1.key), "snapshots are namespaced into pm_backups__<store folder>");

    await sleep(3);
    store.create("task", { title: "Beta" });
    await store.save();
    await sleep(3);
    const snap2 = await B.takeSnapshot(store);

    let snaps = await B.listSnapshots(store);
    ok(snaps.length === 2, "listSnapshots sees both snapshots (" + snaps.length + ")");
    ok(snaps[0].key === snap2.key && snaps[1].key === snap1.key, "listSnapshots is newest-first");
    ok(snaps[0].count === 2 && snaps[0].size > 0, "listed snapshot reports record count + size");

    // auto-snapshot throttle (BEFORE the prune loop)
    localStorage.setItem(lsKey, String(Date.now()));
    const beforeKeys = new Set((await B.listSnapshots(store)).map((s) => s.key));
    B.maybeAutoSnapshot(store);
    await sleep(60);
    const afterKeys = new Set((await B.listSnapshots(store)).map((s) => s.key));
    ok(afterKeys.size === beforeKeys.size, "maybeAutoSnapshot is throttled within 5 minutes");
    localStorage.removeItem(lsKey);
    B.maybeAutoSnapshot(store);
    let grew = false;
    for (let i = 0; i < 30 && !grew; i++) { await sleep(20); grew = (await B.listSnapshots(store)).length > afterKeys.size; }
    ok(grew, "maybeAutoSnapshot takes a snapshot once the throttle window passes");
    ok(Number(localStorage.getItem(lsKey)) > 0, "throttle timestamp persisted to localStorage for the next reload");

    // restore replaces current data with the snapshot
    await B.restoreSnapshot(store, snap1.key);
    ok(store.count("task") === 1 && store.all("task")[0].title === "Alpha", "restoreSnapshot replaces current records with the snapshot");

    // delete a single snapshot
    await B.deleteSnapshot(store, snap1.key);
    snaps = await B.listSnapshots(store);
    ok(!snaps.some((s) => s.key === snap1.key), "deleteSnapshot removes a single snapshot");

    // pruning keeps at most MAX_SNAPSHOTS (30)
    for (let i = 0; i < 31; i++) { await B.takeSnapshot(store); await sleep(3); }
    snaps = await B.listSnapshots(store);
    ok(snaps.length <= 30, "snapshot pruning caps the history at 30 (" + snaps.length + ")");
    ok(snaps.length >= 1, "snapshots remain after pruning");
...
// ── Phase 4 tests (Roadmap 18): quick capture ────────────────────
export async function runQuickCaptureTests() {
  const results = [];
  const ok = (cond, name, extra) => results.push({ ok: !!cond, name, extra });
  const Q = await import("./quickcapture.js");
  const t = todayLocal();
  const folder = "pm_test_qc_" + Math.random().toString(36).slice(2, 9);
  const store = new Store({ kv: root.kv, folder, debounceMs: 5 });
  const cleanup = async (s) => { for (const [k] of await s.kvFolder.entries()) await s.kvFolder.delete(k); };
  try {
    await store.load();
    ...
    // parser edges
    const m = Q.parseQuickInput("write the plan #alpha #beta p:low");
    ok(m && JSON.stringify(m.tags) === JSON.stringify(["alpha", "beta"]), "parseQuickInput: collects multiple #tags");
    ok(m && m.priority === "low" && m.title === "write the plan", "parseQuickInput: p:low sets priority and strips the token");
    const n = Q.parseQuickInput("n: jot this down");
    ok(n && n.type === "note" && n.title === "jot this down", "parseQuickInput: note prefix");
    const ev = Q.parseQuickInput("e: standup");
    ok(ev && ev.type === "event" && ev.date === t && ev.startTime === "", "parseQuickInput: event without a time defaults to today, no start time");
    const ev2 = Q.parseQuickInput("e: standup 14:00");
    ok(ev2 && ev2.startTime === "14:00" && ev2.title === "standup", "parseQuickInput: event with a time");
    ok(Q.parseQuickInput("") === null && Q.parseQuickInput("   ") === null, "parseQuickInput: empty/whitespace is null");
    ok(Q.parseQuickInput("@proj #tag") === null, "parseQuickInput: tokens only (no title) is null");

    // commit writes into local state immediately
    store.create("project", { name: "Work", status: "Active", color: "#22d3ee" });
    const taskRec = Q.commitQuickCapture(store, Q.parseQuickInput("ship the release tomorrow @Work p:high #v1"));
    ok(taskRec && taskRec.type === "task", "commitQuickCapture returns the created task");
    ok(store.count("task") === 1 && store.all("task")[0].id === taskRec.id, "task is in local state immediately (before any save)");
    ok(store.get("project", taskRec.projectId).name === "Work", "commitQuickCapture resolves @project to a project id");
    ok(taskRec.due === addDays(t, 1) && taskRec.priority === "high" && JSON.stringify(taskRec.tags) === JSON.stringify(["v1"]), "task keeps its due date, priority, and tags");
    const orphan = Q.commitQuickCapture(store, Q.parseQuickInput("loose task @nope"));
    ok(orphan && orphan.projectId === null, "unknown @project resolves to null instead of crashing");
    const noteRec = Q.commitQuickCapture(store, Q.parseQuickInput("n: an idea"));
    const eventRec = Q.commitQuickCapture(store, Q.parseQuickInput("e: sync 10:00 tomorrow"));
    ok(noteRec && noteRec.type === "note" && eventRec && eventRec.type === "event", "notes and events commit as their own entity types");
    ok(eventRec.date === addDays(t, 1) && eventRec.startTime === "10:00", "event commit keeps parsed date + time");

    // survives save + reload (persisted locally)
    await store.save();
    const store2 = new Store({ kv: root.kv, folder });
    await store2.load();
    ok(store2.count("task") === 2 && store2.count("note") === 1 && store2.count("event") === 1, "captured records persist through kv on reload");
    ok(store2.get("task", taskRec.id).title === "ship the release", "captured task data round-trips intact");

    await cleanup(store2);
  } catch (e) {
    ok(false, "test run threw: " + e.message, e.stack);
  }
  const passed = results.filter((r) => r.ok).length;
...
// ── Phase 5 tests (Roadmap 21/23/24): Project-U registry, shared ids, links ──
export async function runIntegrateTests() {
  const results = [];
  const ok = (cond, name, extra) => results.push({ ok: !!cond, name, extra });
  try {
    const I = await import("./integrate.js");

    // member registry
    const ids = I.MEMBERS.map((m) => m.id);
    ok(new Set(ids).size === ids.length, "member registry ids are unique (" + ids.length + " members)");
    ok(ids.includes("pm-u"), "pm-u is in the family registry");
    ok(["crm-u", "psa-u", "quote-u", "it-u", "rmm-u"].every((x) => ids.includes(x)), "all five referenced members are registered");
    ok(I.MEMBERS.every((m) => /^https:\/\/perchance\.org\/[a-z0-9-]+$/.test(I.memberUrl(m.id))), "every member resolves a top-level perchance URL");
    ok(I.member("crm-u") && I.member("crm-u").title === "CRM-U", "member() looks up by id");
    ok(I.member("nope") === null, "member() returns null for an unknown id");
    ok(I.isKnownEntity("crm-u", "account") && !I.isKnownEntity("crm-u", "spaceship"), "isKnownEntity distinguishes known types");
    ok(I.entityLabel("it-u", "organization") === "Organization", "entityLabel resolves a human label");
    ok(I.members({ includeSelf: false }).every((m) => m.id !== "pm-u"), "members({includeSelf:false}) excludes pm-u");

    // shared ids
    const sid = I.mintSharedId("pm-u", "company");
    ok(I.isSharedId(sid) && sid.startsWith("puid~pm-u~company~"), "mintSharedId builds the family scheme");
    const parsed = I.parseSharedId(sid);
    ok(parsed && parsed.app === "pm-u" && parsed.kind === "company" && parsed.uid.length > 0, "parseSharedId round-trips app/kind/uid");
    ok(I.sharedIdApp(sid) === "pm-u" && I.sharedIdKind(sid) === "company", "sharedIdApp/sharedIdKind helpers");
    ok(I.parseSharedId("garbage") === null && I.parseSharedId("puid~x") === null && I.parseSharedId("") === null, "parseSharedId rejects malformed ids");
    ok(I.mintSharedId("pm-u", "company") !== sid, "minted ids are unique");

    // deep links
    const href = I.recordHref({ app: "crm-u", type: "account", id: sid });
    ok(href.startsWith("https://perchance.org/crm-u#/") && href.includes("ref=" + encodeURIComponent(sid)), "recordHref builds a read-only deep link with the shared id");
    ok(!I.recordHref({ app: "crm-u" }).includes("ref="), "recordHref without an id omits the ref param");
    ok(I.recordHref({}) === null, "recordHref needs an app");
    const n = I.normalizeRef({ app: "crm-u", type: "account", id: sid });
    ok(n && n.app === "crm-u" && n.id === sid && n.label, "normalizeRef keeps a valid reference");
    ok(I.normalizeRef({ app: "nonexistent-u", id: sid }) === null && I.normalizeRef({ app: "crm-u" }) === null, "normalizeRef rejects unknown members / missing ids");
    ok(I.refKey(n) === "crm-u/account/" + sid, "refKey is a stable identity for de-duping");
    ok(I.refHref(n) === href, "refHref matches recordHref");

    // identity cross-links
    const targets = I.identityTargets("company");
    ok(targets.some((t) => t.app === "crm-u" && t.type === "account"), "a company links to a CRM-U account");
    const links = I.identityLinks("company", sid);
    ok(links.length > 0 && links.every((l) => l.href.includes(encodeURIComponent(sid))), "identityLinks returns read-only links carrying the shared id");

    // member descriptor (task 21)
    const d = I.buildMemberDescriptor({ title: "PM-U" });
    ok(d.$schema === "project-u/member-descriptor@1" && d.id === "pm-u" && d.family === "Project U", "member descriptor carries schema/id/family");
    ok(d.url === "https://perchance.org/pm-u" && Array.isArray(d.entities) && d.entities.length >= 9, "member descriptor exposes url + entities");
    ok(d.sharedIdScheme === I.SHARED_ID_SCHEME && d.refParam === "ref", "member descriptor documents the shared-id scheme + ref param");
    ok(I.buildMemberDescriptor({ accent: "#123456" }).accent === "#123456", "member descriptor accepts identity overrides");
  } catch (e) {
    ok(false, "test run threw: " + e.message, e.stack);
  }
  const passed = results.filter((r) => r.ok).length;
...
// ── Phase 5 tests (Roadmap 22/23): shared identities + cross-app refs ──
export async function runIdentityTests() {
  const results = [];
  const ok = (cond, name, extra) => results.push({ ok: !!cond, name, extra });
  const I = await import("./integrate.js");
  const ID = await import("./identities.js");
  const folder = "pm_test_id_" + Math.random().toString(36).slice(2, 9);
  const store = new Store({ kv: root.kv, folder, debounceMs: 5 });
  const cleanup = async (s) => { for (const [k] of await s.kvFolder.entries()) await s.kvFolder.delete(k); };
  try {
    await store.load();

    // create identities — each gets a global shared id
    const co = ID.createIdentity(store, "company", { name: "Acme Ltd", domain: "acme.com" });
    ok(co && co.type === "company" && co.name === "Acme Ltd", "createIdentity makes a company record");
    ok(I.isSharedId(co.sharedId) && I.sharedIdKind(co.sharedId) === "company", "company carries a puid company shared id");
    const cust = ID.createIdentity(store, "customer", { name: "Jane Doe" });
    ok(cust && cust.type === "customer" && I.sharedIdKind(cust.sharedId) === "customer", "createIdentity makes a customer with a customer shared id");
    ok(store.count("company") === 1 && store.count("customer") === 1, "identities are first-class store records");
    ok(ID.createIdentity(store, "widget", { name: "no" }) === null, "createIdentity rejects an unknown kind");

    // ensureIdentity — adopt a foreign id, idempotently
    const foreign = I.mintSharedId("pm-u", "company", "acme-acme-acme");
    ok(ID.ensureIdentity(store, foreign, { name: "Acme Ltd" }).sharedId === foreign, "ensureIdentity adopts a shared id");
    ok(store.count("company") === 2, "adopted identity becomes a local record");
    ID.ensureIdentity(store, foreign, { name: "Acme Renamed" });
    ok(store.count("company") === 2 && ID.identityBySharedId(store, foreign).name === "Acme Renamed", "ensureIdentity de-dupes by shared id and updates the name");
    ok(ID.ensureIdentity(store, "puid~crm-u~account~zzz", {}) === null, "ensureIdentity refuses a non-identity shared id");
    ok(ID.identityBySharedId(store, co.sharedId).id === co.id, "identityBySharedId finds the local record");

    // link a project to a company + customer (task 22)
    const proj = store.create("project", { name: "Website", status: "Active", companyId: co.id, customerId: cust.id });
    ok(ID.identityProjects(store, co).some((p) => p.id === proj.id), "identityProjects finds the linked project");
    ok(ID.identityProjects(store, cust).length === 1, "the customer link is tracked too");
    ok(ID.identityLinksFor(co).length > 0, "identityLinksFor returns cross-app links for a company");

    // cross-app references on the project (task 23)
    const target = I.mintSharedId("crm-u", "account", "target-account-1");
    const withRef = ID.addRef(store, "project", proj.id, { app: "crm-u", type: "account", id: target, label: "Acme account" });
    ok(withRef && ID.recordRefs(withRef).length === 1, "addRef stores a cross-app reference on the project");
    ok(ID.recordRefs(store.get("project", proj.id))[0].href === undefined, "stored references hold data, not computed links");
    ok(ID.addRef(store, "project", proj.id, { app: "crm-u", type: "account", id: target }).refs.length === 1, "adding the same reference twice is a no-op");
    const bad = ID.addRef(store, "project", proj.id, { app: "nope-u", type: "x", id: "y" });
    ok(bad === null && ID.recordRefs(store.get("project", proj.id)).length === 1, "an invalid reference is rejected without touching the record");
    const all = ID.allRefs(store);
    ok(all.length === 1 && all[0].ownerType === "project" && all[0].ownerLabel === "Website", "allRefs annotates the owning record");
    ok(I.recordHref(all[0]).includes(encodeURIComponent(target)), "a stored reference resolves to a read-only deep link");

    // persistence through kv (schema v2)
    await store.save();
    const store2 = new Store({ kv: root.kv, folder });
    await store2.load();
    ok(store2.count("company") === 2 && store2.count("customer") === 1, "identities persist across reload");
    ok(ID.recordRefs(store2.get("project", proj.id)).length === 1, "cross-app references persist across reload");
    ok(store2.get("project", proj.id).companyId === co.id, "project identity link persists across reload");
    const dump = store2.exportAll();
    let backupOk = true;
    try { store2.validateBackup(dump); } catch (e) { backupOk = false; }
    ok(backupOk && dump.schemaVersion === SCHEMA_VERSION && Array.isArray(dump.entities.company), "a v" + SCHEMA_VERSION + " backup validates with identity entities");

    // inbound deep-link adoption
    const inbound = I.mintSharedId("pm-u", "customer", "inbound-cust-1");
    const adopted = ID.importIdentityFromParams(store2, { ref: inbound, name: "Inbound Co" });
    ok(adopted && adopted.sharedId === inbound && adopted.name === "Inbound Co", "importIdentityFromParams adopts an inbound identity link");
    ok(ID.importIdentityFromParams(store2, { ref: "not-an-id" }) === null, "importIdentityFromParams ignores a bad ref");

    // removeRef
    ID.removeRef(store2, "project", proj.id, I.refKey({ app: "crm-u", type: "account", id: target }));
    ok(ID.recordRefs(store2.get("project", proj.id)).length === 0, "removeRef removes a single reference");
...
// ── Phase 6 tests: project-master migration (tasks 26–27) ────────
// Feeds a realistic (and deliberately messy) project-master v1 backup through
// the whole pipeline: detect → plan (normalise + repair) → validate → apply,
// and checks that nothing touches the network or the user's real data.
export async function runMigrationTests() {
  const results = [];
  const ok = (cond, name, extra) => results.push({ ok: !!cond, name, extra });
  const folder = "pm_test_mig_" + Math.random().toString(36).slice(2, 9);
  const store = new Store({ kv: root.kv, folder, debounceMs: 5 });
  ...
  try {
    ...
    const v1 = {
      ...
      schemaVersion: 1,
      exportedAt: 111,
      settings: { profileName: "  Sam  ", theme: "light", focusWork: 50, focusShort: "7", junk: true },
      entities: {
        project: [
          {
            id: "p1", type: "project", name: "Website", status: "Active",
            milestones: [{ id: "m1", name: "Launch", due: "2026-09-30", taskIds: ["t1", "gone"] }, { name: "No id" }],
            ideas: [{ text: "Add a blog" }],
          },
          { name: "no id project" },
        ],
        task: [
          {
            id: "t1", type: "task", title: "Build", projectId: "p1", milestoneId: "m1", status: "Active",
            tags: "urgent", due: "2026-09-20", created: 1000, subtasks: [{ title: "S1" }], dependsOn: ["t2", "gone2"],
            timeLog: [{ id: "x", start: 5, end: 9 }],
            refs: [{ app: "crm-u", type: "account", id: "puid~crm-u~account~a" }],
            recurrence: { freq: "weekly" },
          },
          { id: "t2", type: "task", title: "Design", projectId: "gone", status: "Active", milestoneId: "nope", deps: ["t1"] },
          { id: "t1", type: "task", title: "DUP", projectId: "p1" },
        ],
        note: [{ id: "n1", type: "note", title: "Notes", body: "x", projectId: "gone", pinned: "true" }],
        habit: [{ id: "h1", type: "habit", name: "Read", history: { "2026-09-01": true, "not-a-date": true, "2026-09-02": false } }],
        focuslog: [{ id: "f1", type: "focuslog", started: 1000, durationMin: "25", taskId: "gone", mode: "work" }],
        checklist: [{ id: "c1", type: "checklist", name: "Check", items: [{ text: "one" }, { id: "i2", text: "two", done: 1 }, { text: "" }] }],
        board: [{ id: "b1", type: "board", name: "Board", kind: "swot", data: {}, projectId: "gone" }],
        event: [{ id: "e1", type: "event", title: "Call", date: "2026-09-15" }],
        mystery: [{ id: "z1" }],
      },
    };

    // detection
    const det = M.detectBackup(v1);
    ok(det && det.source === "project-master" && det.fromSchema === 1, "detectBackup recognises a project-master v1 envelope");
    ok(det.needsUpgrade === true && det.isCurrent === false, "detectBackup flags a v1 payload as needing upgrade");
    ok(M.detectBackup({ app: "nope", entities: {} }) === null && M.detectBackup(null) === null && M.detectBackup({ app: "pm-u" }) === null, "detectBackup rejects foreign / malformed payloads");

    // plan
    const { payload: mp, report } = M.planMigration(v1);
    ok(report.fromSchema === 1 && report.toSchema === SCHEMA_VERSION, "plan records the schema upgrade");
    ok(mp.app === "pm-u" && mp.schemaVersion === SCHEMA_VERSION, "plan emits a current pm-u payload");
    ok(mp.migratedFrom.app === "project-master", "plan keeps migration provenance");

    // settings
    ok(mp.settings.profileName === "Sam" && mp.settings.theme === "light", "settings coerced (trim + theme)");
    ok(mp.settings.focusWork === 50 && mp.settings.focusShort === 7, "settings focus durations coerced to numbers");
    ok(!("junk" in mp.settings), "settings unknown keys dropped");

    // counts / skips / warnings
    ok(report.counts.project === 1 && report.counts.task === 2, "counts: id-less project dropped, duplicate task dropped");
    ok(report.skipped.some((s) => s.reason === "missing id") && report.skipped.some((s) => s.reason === "duplicate id"), "report lists skipped records (missing + duplicate id)");
    ok(report.warnings.some((w) => w.includes("mystery")) && !("mystery" in mp.entities), "unknown entity type warned about + dropped");
    ok(report.addedTypes.includes("company") && report.addedTypes.includes("customer"), "report notes the new v2 entity types");
    ok(mp.entities.company.length === 0 && mp.entities.customer.length === 0, "v1 backup gains empty company/customer collections");

    // repairs
    const t1 = mp.entities.task.find((t) => t.id === "t1");
    const t2 = mp.entities.task.find((t) => t.id === "t2");
    ok(t1.title === "Build", "duplicate id keeps the first occurrence");
    ok(t2.projectId === null && t2.milestoneId === null, "dangling project + milestone ids cleared");
    ok(mp.entities.note[0].projectId === null && mp.entities.board[0].projectId === null, "dangling note/board project links cleared");
    ok(mp.entities.focuslog[0].taskId === null, "dangling focuslog task link cleared");
    ok(mp.entities.project[0].milestones[0].taskIds.join() === "t1", "milestone taskIds pruned to existing tasks");
    ok(t1.dependsOn.join() === "t2", "dependency ids pruned to existing tasks");
    ok(!("deps" in t1) && !("deps" in t2), "legacy `deps` alias folded into the canonical dependsOn field");
    ok(t2.dependsOn.join() === "t1", "legacy `deps` alias preserved as dependsOn");
    ok(report.repairs.length >= 5, "report lists each repaired reference");

    // record normalisation
    ok(Array.isArray(t1.tags) && t1.tags[0] === "urgent", "scalar tags field becomes a list");
    ok(t1.subtasks[0].id && t1.subtasks[0].title === "S1", "subtasks gain ids");
    ok(t1.timeLog.length === 1 && t1.timeLog[0].end === 9, "timeLog entries preserved");
    ok(t1.recurrence.freq === "weekly" && t1.recurrence.interval === 1, "recurrence defaulted");
    ok(t1.refs.length === 1 && t1.refs[0].app === "crm-u", "cross-app refs preserved + normalised");
    ok(mp.entities.note[0].pinned === true, "truthy strings coerce to booleans");
    ok(Object.keys(mp.entities.habit[0].history).join() === "2026-09-01", "habit history keeps only true ISO days");
    ok(mp.entities.checklist[0].items.length === 2 && mp.entities.checklist[0].items.every((i) => i.id), "checklist items gain ids, blanks dropped");
    ok(mp.entities.project[0].milestones.length === 2 && mp.entities.project[0].milestones[1].id, "milestones gain ids");

    // validation
    const check = M.validateMigratedPayload(mp);
    ok(check.ok === true && check.warnings.length === 0, "a planned payload validates cleanly");
    const bad = JSON.parse(JSON.stringify(mp));
    bad.entities.task[0].type = "project";
    ok(M.validateMigratedPayload(bad).ok === false, "validation: mismatched record type is an error");
    const bad2 = JSON.parse(JSON.stringify(mp));
    bad2.entities.task.push(JSON.parse(JSON.stringify(bad2.entities.task[0])));
    ok(M.validateMigratedPayload(bad2).ok === false, "validation: duplicate ids are an error");

    let threw = false; try { M.planMigration({ app: "nope", entities: {} }); } catch (e) { threw = true; }
    ok(threw, "plan rejects a foreign envelope");
    threw = false; try { M.planMigration({ app: "pm-u", schemaVersion: SCHEMA_VERSION + 5, entities: {} }); } catch (e) { threw = true; }
    ok(threw, "plan rejects a future schema");

    // apply — merge
    store.create("project", { id: "p1", name: "Local website" });
    store.create("task", { id: "keep", title: "Untouched" });
    const merged = M.applyMigration(store, mp, { mode: "merge" });
    ok(merged.added > 0 && merged.updated === 1, "merge adds new records and updates the colliding id");
    ok(store.get("project", "p1").name === "Website", "merge: imported record wins an id clash");
    ok(store.get("task", "keep").title === "Untouched", "merge: unrelated local records survive");
    ok(store.count("task") === 3, "merge: imported tasks added alongside local ones");

    // apply — replace
    const replaced = M.applyMigration(store, mp, { mode: "replace" });
    ok(replaced.removed >= 1 && store.get("task", "keep") === null, "replace clears records absent from the backup");
    ok(store.count("project") === 1 && store.count("task") === 2, "replace loads exactly the migrated set");

    // invalid payloads never touch data
    threw = false; try { M.applyMigration(store, bad); } catch (e) { threw = true; }
    ok(threw && store.count("task") === 2, "apply refuses an invalid payload without changing data");
    ...
    const origSend = XMLHttpRequest.prototype.send;
    const origBeacon = navigator.sendBeacon;
    window.fetch = function (input, init) { try { captured.push(typeof input === "string" ? input : (input && input.url) || ""); if (init && init.body) captured.push(String(init.body)); } catch (e) {} return origFetch.apply(this, arguments); };
    XMLHttpRequest.prototype.open = function (method, url) { try { this.__url = url; } catch (e) {} return origOpen.apply(this, arguments); };
    XMLHttpRequest.prototype.send = function (body) { try { captured.push(this.__url || ""); if (body) captured.push(String(body)); } catch (e) {} return origSend.apply(this, arguments); };
    if (origBeacon) navigator.sendBeacon = function (url, data) { try { captured.push(String(url)); if (data) captured.push(String(data)); } catch (e) {} return origBeacon.apply(this, arguments); };
    try {
      M.applyMigration(store, mp, { mode: "merge" });
      await store.save();
      ok(captured.length === 0, "migration issues zero network requests (" + captured.length + " seen)");
    } finally {
      window.fetch = origFetch;
      XMLHttpRequest.prototype.open = origOpen;
      XMLHttpRequest.prototype.send = origSend;
    ...
    // report markup
    const html = M.migrationReportHTML(report);
    ok(html.includes("schema v1") && html.includes("task") && html.includes(String(report.total)), "report markup includes the schema line + counts");

    // cleanup
    store.wipeAll();
    await store.save();
    const v = new Store({ kv: root.kv, folder });
    await v.load();
    ok(v.records.size === 0, "cleanup emptied the throwaway test folder");
  } catch (e) {
    ok(false, "test run threw: " + e.message, e.stack);
  }
  const passed = results.filter((r) => r.ok).length;
