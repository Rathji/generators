...
import { textInput, textareaInput, selectInput, swatchField, fieldStack, fieldGrid, formModal, submitOnEnter, focusFirst } from "./formkit.js";
...
import { noteCardHTML, openNoteEditor, wireNoteCards } from "./notes.js";
...
import * as idmod from "./identities.js";
import { member, recordHref } from "./integrate.js";
...
// ── create/edit project modal (task 18) — template-u input suite ──
export function openProjectModal(store, { project = null } = {}) {
  ...
  const colorField = swatchField({ label: "Color", name: "color", options: PROJECT_COLORS, value: project ? project.color : PROJECT_COLORS[0] });
  const statusField = selectInput({ name: "status", label: "Status", value: (project && project.status) || "Active", options: PROJECT_STATUSES });
  const targetField = textInput({ name: "targetDate", label: "Target date", type: "date", value: project ? project.targetDate : "" });
  const descField = textareaInput({ name: "description", label: "Description", value: project ? project.description : "", placeholder: "What is this project about?" });

  // Project-U shared identities (Phase 5) — pick a company/customer minted in
  // the Project U view; both travel by global shared id.
  const companies = idmod.identities(store, "company").map((c) => ({ value: c.id, label: c.name }));
  const customers = idmod.identities(store, "customer").map((c) => ({ value: c.id, label: c.name }));
  const companyField = selectInput({ name: "companyId", label: "Company (Project U)", placeholder: companies.length ? "— none —" : "Add companies in Project U", options: companies, value: project ? project.companyId || "" : "" });
  const customerField = selectInput({ name: "customerId", label: "Customer (Project U)", placeholder: customers.length ? "— none —" : "Add customers in Project U", options: customers, value: project ? project.customerId || "" : "" });

  const save = () => {
    const name = String(nameField.value || "").trim();
    if (!name) {
      nameField.setError("Please give the project a name");
      nameField.focus();
      return false;
    }
    store.upsert("project", project ? project.id : uid(), {
      ...
      targetDate: targetField.value || "",
      description: String(descField.value || "").trim(),
      companyId: companyField.value || null,
      customerId: customerField.value || null,
    });
  ...
  const { el } = formModal({
    ...
    subtitle: isEdit ? "Update the project details." : "Set up a project to organize its tasks, milestones and plans.",
    className: "proj-modal",
    body: [fieldStack(nameField, colorField, statusField, targetField, descField, fieldGrid(companyField, customerField))],
    acceptLabel: isEdit ? "Save changes" : "Create project",
    onAccept: save,
  });
...
export function projectWorkspaceHTML(store, project, tab) {
  ...
  const body = tab === "overview" ? overviewHTML(store, project, st)
    ...
    : tab === "notes" ? projectNotesHTML(store, project)
    : overviewHTML(store, project, st);
  return `
...
function projectNotesHTML(store, project) {
  const pmap = new Map(store.all("project").map((p) => [p.id, p]));
  const notes = store
    .all("note")
    .filter((n) => n.projectId === project.id)
    .sort((a, b) => (!!a.pinned !== !!b.pinned ? (a.pinned ? -1 : 1) : (b.updated || b.created || 0) - (a.updated || a.created || 0)));
  const cards =
    notes.map((n) => noteCardHTML(store, pmap, n)).join("") ||
    `<div class="proj-empty">${ICONS.file}<h2>No notes in this project</h2><p>Notes you attach to “${esc(project.name)}” appear here — and in the global Notes view.</p></div>`;
  return `
    <div class="view-head">
      <div class="view-head-top">
        <div><h1 style="font-size:1.15rem;"><span class="vh-ico">${ICONS.file}</span> Notes</h1><p class="sub">${notes.length} note${notes.length === 1 ? "" : "s"} attached to this project.</p></div>
        <button class="btn btn-primary" id="pnNewBtn">${ICONS.plus} New note</button>
      </div>
    </div>
...
// Read-only Project-U links for a project: linked company/customer identities
// and any cross-app references. Reference-not-copy — each link opens the owner.
function ecoLinksHTML(store, project) {
  const bits = [];
  const chip = (rec, icon) => {
    const links = idmod.identityLinksFor(rec);
    return `<div class="eco-mini">
      <span class="eco-mini-ico">${ICONS[icon]}</span>
      <span class="eco-mini-name">${esc(rec.name)}</span>
      <span class="eco-mini-links">${links.map((l) => `<a class="eco-link" href="${esc(l.href)}" target="_blank" rel="noopener" title="Open in ${esc(l.title)}">${esc(l.title)}${ICONS.external}</a>`).join("")}</span>
    </div>`;
  };
  const company = project.companyId ? store.get("company", project.companyId) : null;
  const customer = project.customerId ? store.get("customer", project.customerId) : null;
  if (company) bits.push(chip(company, "building"));
  if (customer) bits.push(chip(customer, "users"));
  for (const r of idmod.recordRefs(project)) {
    const m = member(r.app);
    bits.push(`<div class="eco-mini">
      <span class="eco-mini-ico">${ICONS.link}</span>
      <span class="eco-mini-name">${esc(r.label || r.id)}</span>
      <span class="eco-mini-links"><a class="eco-link" href="${esc(recordHref(r))}" target="_blank" rel="noopener" title="Open read-only">${esc(m ? m.title : r.app)}${ICONS.external}</a></span>
    </div>`);
  }
  return bits.length ? bits.join("") : `<p class="ws-empty">No company, customer or cross-app references linked yet — add them in <b>Project U</b>.</p>`;
}
      ...
      <div>
        <div class="ov-col">
          ...
          <div class="milestone-add">
            ...
            <input type="date" id="msDueInput" title="Milestone due date" aria-label="Milestone due date">
        ...
        <div class="ov-col">
          <h2>Project U links</h2>
          <p class="sub">Companies, customers and cross-app records shared by global id — read-only.</p>
          ${ecoLinksHTML(store, project)}
          <a class="btn" href="#/ecosystem" style="margin-top:10px;">${ICONS.network} Manage in Project U</a>
        </div>
      </div>
    </div>`;
}
...
export function wireProjectWorkspace(store, project, ctx) {
  ...
  // project notes tab (task 36 — previously a stub)
  $("#pnNewBtn")?.addEventListener("click", () => openNoteEditor(store, { defaults: { projectId: project.id } }));
  wireNoteCards(store, document.querySelector(".ws-body") || document);
}
