...
//
// PM-U integration (Phase 3, task 11): the task picker and the start/reset
// controls are built from the template-u shared component suite (via formkit),
// and every piece of feedback goes through the template-u toaster — session
// completion uses a titled toast with an inline "start now" action.
//
// Pure helpers (modeDurations, sessionAdvance, focusToday, focusTotals) covered
// by runPhase8Tests().

import { $, esc, notify, appTitle } from "./ui.js";
import { selectInput, actionButton, pmIcon } from "./formkit.js";
...
export function focusViewHTML(store) {
  ...
  // Template-u select field (task 11) — placeholder doubles as "no task attached".
  const taskField = selectInput({
    name: "fzTask",
    label: "Focus on a task",
    placeholder: "No task attached",
    options: tasks.map((t) => ({ value: t.id, label: t.title })),
    value: focusState.taskId || "",
  });
  ...
  const startBtn = actionButton({ label: focusState.running ? "Pause" : "Start", variant: "primary", icon: focusState.running ? "pause" : "play", id: "fzStartBtn" });
  const resetBtn = actionButton({ label: "", variant: "secondary", icon: "refresh", id: "fzResetBtn", title: "Reset this session" });
  return `
    ...
    <div class="fz-layout">
      <div class="fz-card">
        ...
        <div class="fz-ring-wrap" id="fzRingWrap">
          <div class="fz-ring-svg" id="fzRingSvg">${ringHTML(pct)}</div>
...
function paintStartButton(btn) {
  if (!btn) return;
  const label = document.createElement("span");
  label.textContent = focusState.running ? "Pause" : "Start";
  btn.replaceChildren(pmIcon(focusState.running ? "pause" : "play", 15), label);
}
...
export function wireFocusView(store, ctx) {
  ...
  const paint = () => {
    ...
    paintStartButton($("#fzStartBtn"));
    document.title = (focusState.running ? fmt(focusState.remainMs) + " · " : "") + MODE_LABEL[focusState.mode] + " — " + appTitle();
  };

  const complete = () => {
    stopTick();
    const finished = focusState.mode;
    const wasWork = finished === "work";
    let durMin = 0;
    if (wasWork) {
      const started = Date.now();
      durMin = Math.max(1, Math.round(focusState.durMs / 60000));
      store.create("focuslog", { started, durationMin: durMin, taskId: focusState.taskId || null, mode: "work" });
      focusState.workDoneInCycle++;
    }
    playChime();
    const next = sessionAdvance(finished, focusState.workDoneInCycle);
    const min = loadMode(store, next);
    notify(
      wasWork
        ? `Logged ${durMin} min of focus. Next up: ${MODE_LABEL[next]} (${min} min).`
        : `${MODE_LABEL[finished]} finished. Next up: ${MODE_LABEL[next]} (${min} min).`,
      {
        variant: wasWork ? "success" : "info",
        title: wasWork ? "Focus session complete" : "Break over",
        ms: 9000,
        action: { label: "Start " + MODE_LABEL[next].toLowerCase(), onClick: () => start() },
      }
    );
    paint();
    redraw();
  ...
  document.querySelector(".fz-attach .pu-select")?.addEventListener("change", (e) => { focusState.taskId = e.target.value; redraw(); });
  paint();
}
