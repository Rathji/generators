...
// v2 adds the Project-U shared identity records (`company`, `customer`) that
// carry a global shared id, plus cross-app `refs` on records (Phase 5).
export const SCHEMA_VERSION = 2;

// Backup envelopes this app will import. `pm-u` is its own; the
// project-master / project-manager markers are the inherited family formats, so
// exports made by the predecessor app still restore without conversion.
export const BACKUP_APPS = ["pm-u", "project-master", "project-manager"];

export const ENTITY_TYPES = ["project", "task", "event", "checklist", "note", "habit", "board", "focuslog", "company", "customer"];
...
export class Store {
  ...
  // Accepts pm-u's own backups plus the Project-Master-family envelopes it
  // inherited (project-master / project-manager), so old exports still import.
  validateBackup(data) {
    if (!data || typeof data !== "object" || !BACKUP_APPS.includes(data.app)) {
