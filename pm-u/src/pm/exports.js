...
export function notesToMD(store) {
  const parts = ["# " + appTitle() + " — Notes export", ""];
...
export function checklistsToMD(store) {
  const parts = ["# " + appTitle() + " — Checklists export", ""];
...
export function icsCalendar(store) {
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//" + appTitle() + "//" + appTitle() + " v1//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
  ];
  const uidDomain = appSlug();
  const stamp = icsStamp();
  for (const e of store.all("event")) {
    if (!e.date) continue;
    lines.push("BEGIN:VEVENT");
    lines.push("UID:pm-event-" + e.id + "@" + uidDomain);
  ...
  for (const t of store.all("task")) {
    ...
    lines.push("UID:pm-task-" + t.id + "@" + uidDomain);
    lines.push("DTSTAMP:" + stamp);
...
export function downloadICS(store) {
  downloadText(appSlug() + "-" + todayLocal() + ".ics", icsCalendar(store));
}
