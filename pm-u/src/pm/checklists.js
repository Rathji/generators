...
import { textInput, formModal, fieldStack, submitOnEnter, focusFirst } from "./formkit.js";
...
// Built on the template-u input suite (src/pm/formkit.js).
export function promptModal(title, label, initial = "") {
  return new Promise((resolve) => {
    const field = textInput({ name: "prompt", label, value: initial, maxlength: 160 });
    let settled = false;
    const finish = (val) => {
      if (settled) return;
      settled = true;
