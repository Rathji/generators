...
import { MEMBERS, memberUrl } from "./integrate.js";
...
export class Palette {
  ...
  // ── commands (task 13) ─────────────────────────────────────────
  // Includes one launch command per sibling Project-U tool (task 24), so the
  // whole family is reachable from the pm-u command palette.
  familyCommands() {
    return MEMBERS.filter((m) => m.id !== "pm-u").map((m) => ({
      id: "launch:" + m.id,
      label: "open " + m.id,
      aliases: ["open " + m.id, "launch " + m.id, m.id],
      icon: "external",
      hint: "launch " + m.title + " in a new tab",
    }));
  }
  ...
  cmdDefsList() {
    return [
      ...
      { id: "help", label: "help", aliases: ["help", "?"], icon: "help", hint: "show instructions for this section" },
      ...this.familyCommands(),
    ];
  }
  ...
  runCommand(cmd, text) {
    ...
    switch (cmd.id) {
      ...
      case "help": if (this.handlers.help) this.handlers.help(); break;
      default:
        if (cmd.id.startsWith("launch:")) window.open(memberUrl(cmd.id.slice(7)), "_blank", "noopener");
        break;
    }
    this.close();
  }
