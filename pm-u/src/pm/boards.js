...
import { textInput, selectInput, fieldStack, formModal, actionButton, submitOnEnter, focusFirst } from "./formkit.js";
...
export function newBoardModal(store, ctx, { type = "" } = {}) {
  const nameField = textInput({ name: "name", label: "Name", placeholder: "e.g. Launch ideas", maxlength: 80, required: true });
  const typeField = selectInput({
    name: "type",
    label: "Type",
    value: type || "mindmap",
    options: BOARD_TYPES.map(([t, n, h]) => ({ value: t, label: `${n} — ${h}` })),
  });
  const descField = textInput({ name: "desc", label: "Description", placeholder: "What's this board for?", maxlength: 120 });

  const { el } = formModal({
    title: "New board",
    subtitle: "Choose a brainstorming tool, then give it a name.",
    body: [fieldStack(nameField, typeField, descField)],
    acceptLabel: "Create",
    onAccept: () => {
      const name = String(nameField.value || "").trim();
      if (!name) {
        nameField.setError("Please enter a board name");
        nameField.focus();
        return false;
      }
      const btype = typeField.value || "mindmap";
      const board = store.create("board", { name, kind: btype, desc: String(descField.value || "").trim(), data: boardDefaultData(btype) });
      toast("Board created", "success");
      if (ctx && typeof ctx.open === "function") ctx.open(board.id);
    },
  });
...
// Guarantee a board has its per-type data object before it is rendered/wired.
// Normal creation always seeds `data`, but a legacy or hand-edited record may
// arrive with none — without this the render helpers read through `board.data`
// and throw. Mutates the (store-owned) record in place so later edits persist.
export function ensureBoardData(board) {
  if (board && !board.data) board.data = boardDefaultData(boardKind(board));
  return board;
}

export function boardViewHTML(store, board) {
  ensureBoardData(board);
...
function boardSettingsModal(store, board, ctx) {
  const nameField = textInput({ name: "name", label: "Name", value: board.name, maxlength: 80, required: true });
  const descField = textInput({ name: "desc", label: "Description", value: board.desc || "", maxlength: 120 });
  ...
  const leftButtons = [
    actionButton({
      ...
      onClick: async () => {
        ...
        ref.close();
        if (ctx && typeof ctx.back === "function") ctx.back();
      },
    }),
  ];
  ...
  const { el, close } = formModal({
    ...
    body: [fieldStack(nameField, descField)],
    leftButtons,
    acceptLabel: "Save",
    onAccept: () => {
      const name = String(nameField.value || "").trim();
      ...
      store.upsert("board", board.id, { name, desc: String(descField.value || "").trim() });
...
function prosconsHTML(store, board) {
  ...
  const col = (kind, label, cls) => `
    <div class="pc-col ${cls}">
      ...
      <div class="cl-add"><input data-pc-input="${kind}" placeholder="Add to ${label.toLowerCase()}…" maxlength="160"><button class="btn" data-pc-add="${kind}" title="Add to ${label.toLowerCase()}" aria-label="Add to ${label}">${ICONS.plus}</button></div>
    </div>`;
...
function swotHTML(store, board) {
  ...
  const quad = (k, label, cls) => `
    <div class="swot-q ${cls}">
      ...
      <div class="cl-add"><input data-swot-input="${k}" placeholder="Add to ${label.toLowerCase()}…" maxlength="160"><button class="btn" data-swot-add="${k}" title="Add to ${label.toLowerCase()}" aria-label="Add to ${label}">${ICONS.plus}</button></div>
    </div>`;
...
function moscowHTML(store, board) {
  ...
  const cols = keys.map(([k, label]) => `
    <div class="mw-col">
      ...
      <div class="cl-add"><input data-mw-input="${k}" placeholder="Add to ${label.toLowerCase()}…" maxlength="160"><button class="btn" data-mw-add="${k}" title="Add to ${label.toLowerCase()}" aria-label="Add to ${label}">${ICONS.plus}</button></div>
    </div>`).join("");
...
function affinityHTML(store, board) {
  ...
  const cols = (d.clusters || []).map((c) => `
    <div class="af-col" data-af-cluster="${c.id}">
      ...
      <div class="cl-add"><input data-af-input="${c.id}" placeholder="Add note to ${esc(c.name)}…" maxlength="160"><button class="btn" data-af-add="${c.id}" title="Add note to ${esc(c.name)}" aria-label="Add note to ${esc(c.name)}">${ICONS.plus}</button></div>
    </div>`).join("");
  return `
