...
export function portfolioViewHTML(store) {
  ...
  return `
    ...
    ${d.projects.length ? `<div class="card pf-ledger-card">
      <h3 style="margin:0 0 4px;">Project ledger</h3>
      <p class="muted small" style="margin:0 0 12px;">The same numbers in a sortable, searchable table — click a row to open the project.</p>
      <div id="pfLedger"></div>
    </div>` : ""}`;
}

// Mount the template-u data table into the ledger card (Phase 2, task 7).
export function mountPortfolioLedger(outlet, store, ctx) {
  const mount = outlet.querySelector("#pfLedger");
  ...
  const columns = [
    {
      ...
      sortValue: (r) => r.name,
      format: (value, row) => {
        const dot = h("span", { class: "pf-led-dot", style: { background: row.color } });
        const name = h("span", { class: "pf-led-name" }, row.name);
        return h("span", { class: "pf-led-proj" }, dot, name);
      },
    },
    { key: "status", label: "Status", sortValue: (r) => r.status },
    { key: "done", label: "Done", align: "right", sortValue: (r) => r.done, format: (v, r) => `${r.done}/${r.total}` },
    { key: "pct", label: "Progress", align: "right", sortValue: (r) => r.pct, format: (v) => `${v}%` },
    { key: "overdue", label: "Overdue", align: "right", sortValue: (r) => r.overdue },
    { key: "ms", label: "Milestones", align: "right", sortValue: (r) => r.msDone, format: (v, r) => (r.ms ? `${r.msDone}/${r.ms}` : "—") },
    { key: "open", label: "Open", align: "right", hideOnMobile: true, sortValue: (r) => r.open },
  ];
  const table = createDataTable({
    ...
    getRowId: (row) => row.id,
    onRowClick: (row) => ctx.navigate("projects", { id: row.id }),
  });
