...
import game from './ksp.js';      // ← SWAP THIS for your game module
...
function onStateChange(prev, next) {
  if (next === STATES.TITLE) {
    ...
    const res = (activeGame.getResult && activeGame.getResult()) || null;
    if (res) $('gameOverTitleEl').textContent = res.title;
    const kids = [
      statLine('FINAL SCORE  ' + pad6(final)),
      statLine('HI-SCORE  ' + pad6(highScore), true),
    ];
    if (res) for (const l of res.lines) kids.push(statLine(l));
    gameOverStatsEl.replaceChildren(...kids);
...
$('howToNoteEl').textContent =
  "You're flying the VGN space-program sim: launch, stage, and circularize into " +
  'orbit around Kerbin. Guidance (G) will auto-fly the gravity turn for you — ' +
  'turn it off to pilot manually for a bonus. Physics is real (2-body + drag).';
