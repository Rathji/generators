...
// Which game runs is set in main.pjs `config.demoGame` (pong | none).
// The Pong module is a full game built on src/physics.js.
const DEMOS = { pong: pongGame };
const blankGame = {
  init() {}, reset() {}, update() {},
...
const activeGame = demoName === 'none' ? blankGame : (DEMOS[demoName] || pongGame);
...
function onStateChange(prev, next) {
  if (next === STATES.TITLE) {
    ...
    const lines = typeof activeGame.getResultText === 'function'
      ? activeGame.getResultText()
      : [['FINAL SCORE  ' + pad6(final)]];
    gameOverStatsEl.replaceChildren(
      ...lines.map(([t, gold]) => statLine(t, gold)),
...
const BOOT_LINES = [
  '> vgn-pong  ·  arcade cabinet v1.0.0',
  '> CRT DISPLAY ............ OK',
  '> PADDLE INTERFACE ....... OK',
  '> BALL PROJECTILE ........ OK',
...
$('howToNoteEl').textContent =
  "The classic arcade paddle game on the VGN cabinet. First to 7 wins — move with " +
  "W/S or the mouse and beat the CPU, or grab a friend for ↑/↓ two-player.";
