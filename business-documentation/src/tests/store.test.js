...
export async function run(opts = {}) {
  const tests = [
    ...
    },
    {
      ...
      fn: async () => {
        if (!opts.live) return; // only run when explicitly requested ({live:true})
        ...
        try {
          ...
          if (e && e.code === "STORAGE_UNAVAILABLE") return; // unsaved preview — skip
          if (msg.includes("editable_requires_saved_generator")) return;
          throw e;
        }
      },
    },
