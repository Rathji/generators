...
export class GameMap {
  ...
  public generateTerrain() {
    ...
    // PORT GUARD: bound total river-generation attempts so a bad random seed can never hang the page.
  ...
  private generateTilePath(options: {
    ...
    if (tilesSet < minTilesSet) {
      for (let i = tilesSet; i < minTilesSet; i++) {
        // PORT GUARD: bail if there are no skipped tiles to fill from.
